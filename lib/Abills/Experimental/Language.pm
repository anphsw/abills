package Abills::Experimental::Language;
use strict;
...
=head1 NAME
  ...
  May be renamed to Abills::Language after accept
...
=head2 new($base_dir)
  ...
  Returns:
    Abills::Experimental::Language instance
...
sub new {
  ...
  if (! defined $base_dir){
    ...
    our $Bin;
    FindBin->import('$Bin');
    # This part will substitute $Bin to ../usr/abills/
    if ($Bin =~ m/\/usr\/abills(\/)/){
...
sub load {
  ...
  my $lang_path = ($module
    ? $self->{base_dir} . "/Abills/modules/$module/lng_$language\.pl"
