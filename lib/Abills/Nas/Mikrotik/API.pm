...
sub check_access {
  ...
  my $result = $self->login(@self{'host', 'admin', 'password', 'port'});
...
sub upload_key {
  ...
  my $cert_filename = 'id_dsa.' . $admin_name . '.pub';
  ...
  $self->execute(
    [
      [ '/file print', { file => $cert_filename } ],
      [ '/file set', { 'contents' => $content, '.id' => $cert_filename }, { } ],
      @add_user_commands,
...
sub mtik_query {
  ...
  if ($queries_attr && $attributes_attr && exists $queries{name} && $cmd =~ /\/set$/){
    ...
    my ($search_ret, @search_res) = $self->mtik_query($search_cmd, {'.proplist' => '.id'}, { name => $queries{name} });
    if ($search_ret > 1){
  ...
  if ( $retval > 1 ) {
    $errstr = $results[0]->{'message'};
    _bp('mtik errornous query', {command => \@command, error =>  $errstr}, $DEBUG_ARGS);
  }
