package Abills::Google_web;

=head1 NAME

  Google to Excel output Functions
...
sub table {
  ...
  $self = {
    ...
    rowcolor  => (defined($attr->{rowcolor})) ? $attr->{rowcolor} : undef,
    ID        => $attr->{ID}
  };
  ...
  open my $fh, '>', \$self->{media_buffer} or die "Failed to open filehandle: $!";
...
sub addtd {
  ...
  for (my $i = 0; $i <= $#row; $i++) {
    ...
    if ($val =~ /\[(.+)\|(.{0,100})\]/xm) {
      ...
      if ($val =~ /^=/xm) { #to prevent writing strings starting with '=' as formulas, because we never actually use formulas
...
sub show {
  ...
  my $res = $self->_export_google_drive($self->{ID}, $self->{media_buffer});

  if ($res->{errno} || $res->{error}) {
    my $errstr = $res->{errstr} || $res->{error}->{message} || 'unknown';
    my $error_template = '%ERROR%: %ERRSTR%';
    return $self->tpl_show($error_template,
      { ERROR => $lang->{ERROR}, ERRSTR => $errstr },
      { ID => $self->{ID}, OUTPUT2RETURN => 1 }
    );
  }

  my $file_name = $res->{name} || 'file';

  # FIXME: require Abills::Templates; templates(...) is not possible.
  my $success_template =<< 'TEXT';
  <div class="d-flex align-items-center justify-content-center vh-100">
    <div class="container text-center">
      <h1 class="text-bold">%SUCCESS%!</h1>
      <p class="lead">%FILE% Google Drive: %FILENAME%</p>
    </div>
  </div>
TEXT

  return $self->tpl_show($success_template,
    { SUCCESS => $lang->{SUCCESS}, FILE => $lang->{FILE}, FILENAME => $file_name },
    { ID => $self->{ID}, OUTPUT2RETURN => 1 }
  );
}

#**********************************************************
=head2 _export_google_drive($base_id, $file_data) - export xls to Google Drive
...
=cut
#**********************************************************
sub _export_google_drive {
  my ($self, $base_id, $file_data) = @_;

  my $upload_url = 'https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart';
  ...
  my $token_result = $Google->access_token();
  if ($token_result->{errno}) {
    return { errno => 10, errstr => 'Access denied' };
  }

  my $access_token = $token_result->{access_token};
  my $DATE_WITH_TIME    = strftime "%Y%m%d_%H%M%S", localtime(time);

  my $metadata = {
    name     => $base_id . '_' . $DATE_WITH_TIME . '.xls',
    parents  => [ $CONF->{EXPORT_GOOGLE_PARENT_ID} ],
    mimeType => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
  };

  my $file_metadata = encode_json($metadata);

  # Construct the raw request body
  my $boundary = '----------'.time;
  my $request_body = <<"[END]";
--$boundary
...
[END]
  ...
  eval {
    ...
    $result = decode_json($res->{_content});
  };

  if ($@) {
    $result = { errno => 20, errstr => $@ };
    $@ = undef;
  }
...
sub tpl_show {
  ...
  if (!$attr->{SOURCE}) {
    while ($tpl =~ /\%(\w+)(\=?)([A-Za-z0-9\_\.\/\\\]\[:\-]{0,50})\%/xmg) {
      my $var       = $1;
      my $delimiter = $2;
      my $default   = $3;

      if ($attr->{SKIP_VARS} && $attr->{SKIP_VARS} =~ /$var/xm) {
      }
...
sub AUTOLOAD {
  ...
  if($function =~ /table_header|progress_bar|fetch|img/xm) {
    return q{};
  }

  if ($function =~ /_include/xm) {
