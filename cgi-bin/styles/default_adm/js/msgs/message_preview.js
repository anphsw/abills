        ...
        function () {
          if (!timeoutId) {
            timeoutId = window.setTimeout(function () {
              timeoutId = null;
              if(!$a.attr('aria-describedby')) showMessagePreview()
            }, 1000);
          }
        },
        function () {
      ...
      if (data['_INFO']) {
        ...
        if (data['_INFO']['__REPLY']){
          ...
          if (replies[last_id]['ADMIN_MSG']){
            // Find last client message (skipping last, because already checked)
            ...
            // If all messages are wrote by admin, show last admin message
            if (!message){
