...
Drawable.prototype = {
  ...
  redrawShell            : function () {
    ...
    if (this.raw.ports_type && typeof this.raw.ports_type === 'object') {
      let startFiber = 1;
      let self = this;
      self.extShells = [];

      this.raw.ports_type.forEach(function(type) {
        let shell = self.shellType(startFiber, type.total, '#5c98ab', type.type_name);
        self.rendered.push(shell);
        self.extShells.push(shell);
        startFiber += type.total;
      });
    }
  ...
  shellType: function(startFiber, fibers, color, text) {
    let typeWidth = (SCHEME_OPTIONS.FIBER_FULL_WIDTH) * fibers;
    let type = paper.rect(
      this.x + SCHEME_OPTIONS.FIBER_FULL_WIDTH * (startFiber - 1) + 3,
      this.y,
      typeWidth,
      this.height
    ).attr({
    ...
    if (text && !this.movable) type.tip = new HTMLTip(text, type.node);
  ...
  initTip                : function (info_hash) {
    ...
    if (!this.movable) this.tip = new HTMLTip(info_hash, this.shell.node);
  },
...
function CommutationControlsAbstract() {
  ...
  this.$straighten_btn = this.$panel.find('button#STRAIGHTEN_BTN');
  ...
  this.$straighten_btn.on('click', function() {
    Object.keys(ACommutation.links_by_id).forEach(key => {
      ACommutation.links_by_id[key].clearGeometry();
    });
  });
...
function computePath(start, end) {
  ...
  if (sameAxis) {
    ...
    if (startFiber.vertical) {
      let centerBetweenPoints = Math.abs(startPoint.y - endPoint.y) / 2;
      let middle_y = startPoint.y > endPoint.y ? endPoint.y + centerBetweenPoints : startPoint.y + centerBetweenPoints;
    ...
    //3-2 points path
    result.push({
      x: startFiber.vertical ? startPoint.x : endPoint.x,
      y: startFiber.vertical ? endPoint.y : startPoint.y
    });
  }
 ...
 * @returns coordinate
 */
function getCloserPoint(p1, p2, value, reverse) {
  var del1 = Math.abs(value / 2 - p1);
  var del2 = Math.abs(value / 2 - p2);

  return (Math.max(del1, del2) === del1) ? (reverse ? p1 : p2) : (reverse ? p2 : p1);
}
