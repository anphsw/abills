...
<script>
  ...
  function GetStreets(data) {
    let street_id = jQuery(data).data('street-id') || '%STREET_ID%';
  ...
  var selected_builds = jQuery('#ADD_LOCATION_ID').attr('value');
  ...
  function feelOptionGroup (select, data, groupKey, groupLabel, optionName) {
    ...
    data.forEach(address => {
      ...
      if (address?.secondName) address[optionName] += ` (${address.secondName})`;
