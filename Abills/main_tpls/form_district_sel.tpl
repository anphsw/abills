...
<script>
  ...
  function initDistrictsSelector() {
    ...
    const url = window['IS_CLIENT_INTERFACE'] ? '/api.cgi/user/addresses/districts' : '/api.cgi/districts';

    new UniversalNestedSelector({
      apiEndpoint: url,
      selectName: 'DISTRICT_SEL',
      identifierField: '%DISTRICT_IDENTIFIER%',
      selectedField: '%DISTRICT_SELECTED%',
      idField: 'ID',
      multipleField: 'DISTRICT_MULTIPLE',
      containerClass: 'district-subselect',
      customEventPrefix: 'district',
      eventId: '%DISTRICT_EVENT_ID%',

      infoButton: {
        enabled: '%DISTRICT_INFO_BTN%',
        baseUrl: '?get_index=form_districts&full=1&chg=0',
        icon: 'fa fa-list-alt p-1',
        urlPattern: /chg=\d+/
      },

      multipleSelection: {
        enabled: jQuery(`[name='DISTRICT_MULTIPLE']`).length > 0,
        fieldPrefix: 'DISTRICT_MULTIPLE',
        checkboxClass: 'form-control-static m-2'
      },

      grouping: {
        enabled: true,
        groupByField: 'parentId',
        groupNameField: 'parentName',
        groupLabelTemplate: (name) => `== ${name} ==`
      },

      typeLabels: {
        enabled: true,
        typesData: districtTypesLang,
        typeIdField: 'typeId',
        separator: '/',
        suffix: ':'
      },
