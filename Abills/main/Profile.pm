...
#**********************************************************
=head2 get_css() - Get files from
=cut
#**********************************************************
sub get_css {
  my ($attr) = @_;

  my $img_url = $attr->{IMG_URL};
  ...
  my $content = web_request('https://support.abills.net.ua/getcss.cgi',
    {
      REQUEST_PARAMS => { img_url => $img_url }
    });

  my $filename = 'custom';
  my $i = 1;

  while(-f $colors_dir .'/'. $filename.'_'. $i . '.css') {
    $i++;
  }

  $colors_dir = "../styles/$html->{HTML_STYLE}/colors/";

  if (open(my $fh, '>', $colors_dir .'/'. $filename.'_'. $i . '.css')) {
    print $fh $content;
    close($fh);

    $html->message('info', $lang{ADD}, "$lang{ADD} '".  $filename.'_'. $i . '.css' ."'\n ".
        $html->button($lang{APPLY}, "index=$index&AWEB_OPTIONS=1&COLORS=".$filename.'_'. $i, { BUTTON => 1 })
    );
  }
  else {
    $html->message('err', $lang{ERROR}, "Can't save '". $colors_dir .'/'. $filename.'_'. $i . '.css' ."' $!");
    return 0;
  }
...
sub admin_profile {
  ...
  if ($FORM{img_css}) {
    if (get_css({ IMG_URL => $FORM{img_css} })) {
    }
  }
  ...
  my @names = ();
  foreach my $color_name ( @contents ) {
    $color_name =~ s/\.css//;
    push (@names, $color_name);
  }
  ...
  if (in_array('Events', \@MODULES)){
    ...
    $events_groups_select = events_group_select({
    ...
    if (defined($new_hash{$parent})) {
      ...
      while (($k, $val) = each %{ $new_hash{$parent} }) {
        ...
        $table->addrow(
          ...
          "$prefix " . $html->button($val, "index=$k") . (($module{$k}) ? ' ('. $module{$k} .')'  : ''),
