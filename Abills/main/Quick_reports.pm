  ...
  my $table = $html->table(
    {
      ...
      ID         => 'USERS_SUMMARY',
      rows       => [
        [ $html->button( $lang{TOTAL}, "index=1" ),
          $users->{TOTAL_USERS}, '' ],

        [ $html->button( $lang{DISABLE}, "index=1" ),
          $users->{DISABLED_USERS}, '' ],

        [ $html->button( $lang{DEBETORS}, "index=1&USERS_STATUS=2" ),
          $users->{DEBETORS_COUNT}, $users->{DEBETORS_SUM} ],
        [ $html->button( $lang{CREDIT}, "index=1&USERS_STATUS=5" ),
