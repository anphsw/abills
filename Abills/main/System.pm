...
sub form_billd_plugins {
  ...
  if ($FORM{add}) {
    ...
    $Billd->change({%FORM});
    if (!$Billd->{errno}) {
      $html->message('info', $lang{CHANGED}, "$lang{CHANGED} $Billd->{ID}");
    }
  }
  elsif ($FORM{chg}) {
  ...
  $table = $html->table(
    {
      width       => '600',
      caption     => "availeble billd plugins",
...
#**********************************************************
# Return holiday array for month
#**********************************************************
sub holiday_array {
  my ($month) = @_;
...
sub form_info_lists {
  ...
  if ($FORM{add}) {
    ...
    if (!$users->{errno}) {
      $html->message('info', $lang{INFO}, "$lang{CHANGED}: $FORM{ID}");
    }
  }
  elsif ($FORM{chg}) {
  ...
  my $table = $html->table(
    {
      width => '100%',
      rows  => [ [ $lists_sel, $html->form_input('SHOW', $lang{SHOW}, { TYPE => 'submit' }) ] ]
    }
  );
  ...
  if (defined($FORM{tt})) {
    ...
    $tarif_plan->{NETS_SEL} =  $html->form_select(
      'NET_ID',
      {
        ...
        SEL_OPTIONS    => { '' => '--' },
        MAIN_MENU_AGRV => "chg=". ($tarif_plan->{NET_ID} || '')
      }
    );
