...
sub del {
  ...
  if (! $Bill->{errno}) {
    ...
    if (! $self->{errno}) {
      ...
      $self->{admin}->{MODULE}=q{};
      $self->{admin}->action_add($user->{UID}, "ID:$id, AMOUNT:$sum, DATE:$date / $comments", { TYPE => 16 });
      $self->{db}{db}->commit();
    }
    else {
      $self->{db}{db}->rollback();
...
FIELDS
  ...
  if ($self->{SEARCH_FIELDS} =~ /u\.|a\./xm || $WHERE =~ /u\.|a\./xm) {
...
EXT_TABLES
  ...
  if ($attr->{INVOICE_NUM}) {
    $EXT_TABLES  .= << "EXT_TABLES";
  LEFT JOIN (SELECT payment_id, invoice_id FROM docs_invoice2payments GROUP BY payment_id) i2p ON (p.id=i2p.payment_id)
...
FIELDS
  ...
  if ($attr->{INTERVAL}) {
    if ($type eq 'HOURS') {
      ...
      $self->{SEARCH_FIELDS} = "ROUND(SUM(p.sum) / COUNT(DISTINCT p.uid), 2) AS arppu,  ROUND(SUM(p.sum) / $system_users, 2) AS arpu,";
    }
    elsif ($type eq 'GID') {
      $date = "u.gid";
      $EXT_TABLE_JOINS_HASH{users}=1;
  ...
  if ($self->{admin}->{DOMAIN_ID} || $attr->{GID} || $attr->{TAGS} || $self->{SEARCH_FIELDS} =~ /gid/ || $WHERE =~ /u.gid/xm) {
    $EXT_TABLE_JOINS_HASH{users} = 1;
  }
...
SQL
  ...
  if ($self->{TOTAL} > 0) {
    $sql = <<"SQL";
SELECT COUNT(DISTINCT p.uid) AS total_users,
...
sub reports_period_summary {
  ...
  if ($self->{admin}->{GID}) {
    $self->{admin}->{GID}=~s/,/;/xg;
    push @WHERE_RULES,  @{ $self->search_expr($self->{admin}->{GID}, 'INT', 'u.gid') };
  }

  if ($self->{admin}->{DOMAIN_ID}) {
    push @WHERE_RULES,  @{ $self->search_expr($self->{admin}->{DOMAIN_ID}, 'INT', 'u.domain_id') };
  }
...
sub payment_type_list {
  ...
  my $admin_payment_types = ($attr->{AID}) ?  (",COUNT(case when admins_payments_types.aid = $attr->{AID} then 1 else null end) AS allowed") :
    "";
...
sub payment_report_admin {
  ...
  my $sql = <<"SQL";
SELECT p.method, COUNT(p.id) AS total, SUM(p.sum) AS sum, COUNT(DISTINCT p.uid) AS total_users
FROM payments p
       LEFT JOIN users u ON (u.uid=p.uid)
       LEFT JOIN admins a ON (a.aid=p.aid)
...
sub payment_default_type {
  ...
  $self->query("UPDATE payments_type SET default_payment = 0;", '0');
