...
sub AUTOLOAD {
  ...
  if ($operation eq 'add'){
    ...
    $self->query2(qq{
      SELECT * FROM maps_$entity_name $WHERE
    ...
    $self->query2(qq{
      SELECT COUNT(*) FROM maps_$entity_name GROUP BY id
    });
...
sub routes_list {
  ...
  $self->query2( "SELECT  $self->{SEARCH_FIELDS}  r.id
...
sub routes_coords_add {
  ...
  if ( $attr->{POINTS} ) {
    ...
    $self->query2( "INSERT INTO maps_routes_coords (routes_id, coordy, coordx)
        VALUES (?, ?, ?);",
      undef,
...
sub wifi_zones_list {
  ...
  $self->query2( "SELECT   id, radius, coordx, coordy
...
sub points_list {
  ...
  $self->query2( " SELECT $self->{SEARCH_FIELDS} mp.id
  FROM maps_points mp
...
sub points_change {
  ...
  if ( $attr->{COORDX} && $attr->{COORDY} && $attr->{ID} ) {
    ...
    my $current_object = $self->query2( "SELECT coord_id FROM maps_points WHERE id=?", undef,
...
sub points_max_ids_for_types {
  ...
  $self->query2("SELECT type_id, COUNT(id) AS max_id FROM maps_points GROUP BY type_id $WHERE;", undef,
    {
      COLS_NAME => 1,
      Bind      => \@BIND_VALUES
    });
...
sub layers_list{
  ...
  $self->query2( "SELECT $self->{SEARCH_FIELDS} id FROM maps_layers $WHERE ORDER BY $SORT $DESC LIMIT $PG, $PAGE_ROWS;", undef, {
    COLS_NAME => 1,
    COLS_UPPER => 1,
    %{ $attr ? $attr : {}}}
  );
...
sub circles_list{
  ...
  $self->query2( "SELECT $self->{SEARCH_FIELDS} id FROM maps_circles $WHERE ORDER BY $SORT $DESC LIMIT $PG, $PAGE_ROWS;", undef, {
    COLS_NAME => 1,
    COLS_UPPER => 1,
    %{ $attr ? $attr : {}}}
  );
...
sub polylines_list{
  ...
  $self->query2( "SELECT $self->{SEARCH_FIELDS} id FROM maps_polylines $WHERE ORDER BY $SORT $DESC LIMIT $PG, $PAGE_ROWS;", undef, {
    COLS_NAME => 1,
    COLS_UPPER => 1,
    %{ $attr ? $attr : {}}}
  );
...
sub polyline_points_list{
  ...
  $self->query2( "SELECT $self->{SEARCH_FIELDS} id FROM maps_polyline_points $WHERE ORDER BY $SORT $DESC;", undef, {
    COLS_NAME => 1,
    COLS_UPPER => 1,
    %{ $attr ? $attr : {}}}
  );
...
sub polyline_points_add {
  ...
  if ( $attr->{POINTS} ) {
    ...
    # Fill multi query
    push ( @MULTI_QUERY,
      map { [ $attr->{POLYLINE_ID}, $_->{COORDX}, $_->{COORDY}] } @points
    );

    $self->query2( "INSERT INTO maps_polyline_points (polyline_id, coordy, coordx)
        VALUES (?, ?, ?);",
      undef,
      { MULTI_QUERY => \@MULTI_QUERY } );
...
sub polygons_list{
  ...
  $self->query2( "SELECT $self->{SEARCH_FIELDS} id FROM maps_polygons $WHERE ORDER BY $SORT $DESC LIMIT $PG, $PAGE_ROWS;", undef, {
    COLS_NAME => 1,
    COLS_UPPER => 1,
    %{ $attr ? $attr : {}}}
  );
...
sub polygon_points_list{
  ...
  $self->query2( "SELECT $self->{SEARCH_FIELDS} id FROM maps_polygon_points $WHERE ORDER BY $SORT $DESC;", undef, {
    COLS_NAME => 1,
    COLS_UPPER => 1,
    %{ $attr ? $attr : {}}}
  );
...
sub texts_list{
  ...
  $self->query2( "SELECT $self->{SEARCH_FIELDS} id FROM maps_text $WHERE ORDER BY $SORT $DESC LIMIT $PG, $PAGE_ROWS;", undef, {
    COLS_NAME => 1,
    COLS_UPPER => 1,
    %{ $attr ? $attr : {}}}
  );
...
sub icons_list{
  ...
  $self->query2( "SELECT $self->{SEARCH_FIELDS} id FROM maps_icons $WHERE ORDER BY $SORT $DESC LIMIT $PG, $PAGE_ROWS;", undef, {
    COLS_NAME => 1,
    COLS_UPPER => 1,
    %{ $attr ? $attr : {}}}
  );
...
sub districts_list {
  ...
  $self->query2( "SELECT $self->{SEARCH_FIELDS} md.district_id
...
#**********************************************************
=head2 distance_between($point1, $point2)
...
#**********************************************************
=head2 point_is_beetween($point1, $point_2, $point_3)

  Checks if point_3 lies beetween point_1 and point_2

=cut
#**********************************************************
sub point_is_beetween {
  return (distance_between($_[0], $_[2]) + distance_between($_[1], $_[2])) - distance_between($_[0], $_[1]) <= 1E-8;
}
...
sub break_polyline {
  ...
  for ( my $i = 1; $i < $points_count; $i++ ) {
    if ( point_is_beetween($points_list->[$i - 1], $points_list->[$i], $new_point ) ) {
      $center = $i;
      last;
    }
  }
  $exit_with_error->('Cant find siblings for point') if ($center == - 1);
  ...
  $exit_with_error->('Cant find points for old polyline') if ($self->{errno});

  $self->points_del($old_polyline->{OBJECT_ID});
  $exit_with_error->('Cant delete object fo old polyline') if ($self->{errno});
  ...
  my $add_polyline_with_points = sub {
    ...
    $exit_with_error->('Cant add object for new polyline') if ($self->{errno});

    # Add new polylines with new object
    my $new_id = $self->polylines_add({ OBJECT_ID => $point_id, LAYER_ID => $old_polyline->{LAYER_ID}});
    $exit_with_error->('Cant add polyline for new polyline') if ($self->{errno});

    # Add points for this polylines
    $self->polyline_points_add({ POLYLINE_ID => $new_id, POINTS => $_[0], REVERSE => 1 });
    $exit_with_error->('Cant add points for new polyline') if ($self->{errno});
  ...
  $exit_with_error->('Cant delete old polyline') if ($self->{errno});

  $self->polyline_points_del({POLYLINE_ID => $old_polyline->{ID}});
  $exit_with_error->('Cant delete old polyline points') if ($self->{errno});
...
#**********************************************************
=head2 mercator_to_meters($x1, $y1, $x2, $y2) - converts Mercator projection distance to meters

  http://www.avislab.com/blog/gps-distance/

=cut
#**********************************************************
sub mercator_to_meters{
  use Math::Trig;
...
sub align_polyline_to_coords {
  ...
  if (distance_between($points[0], $new_point) > distance_between($points[$#points], $new_point)) {
