...
sub tariff_change{
  ...
  $attr->{HOT_DEAL} = 0 if (!$attr->{HOT_DEAL});
...
sub  user_tariff_add {
  ...
  my $sql = <<"SQL";
    INSERT INTO abon_user_list SET
      uid = ?,
      tp_id = ?,
      comments = ?,
      date = $date,
...
sub user_tariff_del{
  ...
  $personal_desc = "TP_ID: ". ($attr->{TP_IDS} || $attr->{TP_ID}) . ($personal_desc ? ": $personal_desc" : '');
...
SQL
  ...
  $self->query("SELECT COUNT(*) AS total FROM abon_categories $WHERE;",
    undef,
    { INFO => 1 }
  );
...
sub tariff_gid_add {
  ...
  $self->query_add('abon_gids', $attr);

  return $self;
}

#**********************************************************
=head2 tariff_gid_del($attr)
...
sub tariff_gid_del {
  ...
  $self->query_del('abon_gids', undef, $attr);
