...
sub messages_list {
  ...
  if ($attr->{PLAN_FROM_DATE}) {
    push @WHERE_RULES, "m.plan_date>='$attr->{PLAN_FROM_DATE}' and m.plan_date<='$attr->{PLAN_TO_DATE}'";
  }
...
(m.subject LIKE '%$attr->{SEARCH_MSGS_BY_WORD}%' OR m.message LIKE '%$attr->{SEARCH_MSGS_BY_WORD}%'
...
WHERE
  ...
  if ($attr->{SEARCH_MSGS}) {
    push @WHERE_RULES, << "WHERE";
(m.subject LIKE '%$attr->{SEARCH_MSGS}%' OR u.id LIKE '%$attr->{SEARCH_MSGS}%'
      OR CONCAT_WS(' ', pi.fio, pi.fio2, pi.fio3) LIKE '%$attr->{SEARCH_MSGS}%')
WHERE
  }

  my $WATCHERS =<< "SLQ";
(SELECT GROUP_CONCAT(DISTINCT admins.name SEPARATOR ', ') FROM msgs_watch mw
      LEFT JOIN admins ON mw.aid = admins.aid WHERE mw.main_msg = m.id GROUP BY mw.main_msg) AS watchers
SLQ
  ...
  my @search_params = (
    ...
    [ 'TEAM_ID',                'INT',    'mtm.team_id',                                                               1 ],
    [ 'EXTERNAL_CHAT_ID',       'INT',    'm.external_chat_id',                                                        1 ],
    [ 'WATCHERS',               'INT',    $WATCHERS,                                                                   1 ]
  );
  ...
  if ($self->{SEARCH_FIELDS} =~ /r\./xm || $WHERE =~ /r\./xm || $attr->{GET_NEW}) {
    my $reply_new = q{};
    if ($attr->{GET_NEW}) {
  ...
  if ($self->{SEARCH_FIELDS} =~ /qrt\./xm || $WHERE =~ /qrt\./xm) {
    $EXT_TABLES .= "\nLEFT JOIN msgs_quick_replys_tags qrt FORCE INDEX FOR JOIN (`msg_id`) ON (m.id=qrt.msg_id)";
  }

  if ($self->{SEARCH_FIELDS} =~ /mtm\./xm) {
    $EXT_TABLES .= "\nLEFT JOIN msgs_team_messages mtm FORCE INDEX FOR JOIN (`message_id`) ON (mtm.message_id=m.id)";
  }

  if ($self->{SEARCH_FIELDS} =~ /mc\./xm || $WHERE =~ /mc\./xm) {
    $EXT_TABLES .= "\nLEFT JOIN msgs_chapters mc ON (m.chapter=mc.id)";
  }

  if ($self->{SEARCH_FIELDS} =~ /ca\./xm || $WHERE =~ /ca\./xm) {
    $EXT_TABLES .= "\nLEFT JOIN admins ca ON (m.closed_aid=ca.aid)";
  }

  if ($self->{SEARCH_FIELDS} =~ /ea\./xm) {
    $EXT_TABLES .= "\nLEFT JOIN employees_works em FORCE INDEX FOR JOIN (`ext_id`) ON (em.ext_id=m.id)";
    $EXT_TABLES .= "\nLEFT JOIN admins ea ON (em.employee_id=ea.aid)";
  }

  if ($self->{SEARCH_FIELDS} =~ /builds\./xm) {
  ...
  if ($admin->{DOMAIN_ID}) {
    ...
    if ($WHERE && $WHERE =~ /u\.domain_id='\d+'/xm) {
      $WHERE =~ s/u\.domain_id='\d+'/(CASE WHEN m.uid=0 THEN m.domain_id IN ($admin->{DOMAIN_ID}) ELSE u.domain_id IN ($admin->{DOMAIN_ID}) END)/xg;
    }
    elsif ($WHERE) {
  ...
  if ($self->{SEARCH_FIELDS} =~ /u\./xm || $EXT_TABLES =~ /u\./xm || $WHERE =~ /u\./xm) {
    $EXT_TABLES = "LEFT JOIN users u ON (m.uid=u.uid)\n" . $EXT_TABLES;
  }

  if ($self->{SEARCH_FIELDS} =~ /g\./xm || $WHERE =~ /g\./xm) {
    $EXT_TABLES .= "  LEFT JOIN `groups` mg ON (m.gid=mg.gid)\n";
  }

  if ($self->{SEARCH_FIELDS} =~ /\s+a\./xm || $WHERE =~ /\s+a\./xm) {
    $EXT_TABLES .= "\nLEFT JOIN `admins` a ON (m.aid=a.aid)";
  }

  if ($self->{SEARCH_FIELDS} =~ /ra\./xm || $WHERE =~ /ra\./xm) {
...
SQL
    ...
    $self->query($sql, 'do', { Bind => \@id_arr });

    $self->query("DELETE FROM msgs_watch WHERE main_msg IN ($msg_ids)",
      'do', { Bind => \@id_arr }
    );
  }
  elsif ($attr->{UID}) {
    $self->query('DELETE FROM msgs_attachments WHERE create_by = ? AND message_id = ?  AND message_type=0 ;',
      'do',
      { Bind => [ $attr->{UID}, $attr->{ID} ] });
  }
...
sub chapters_list {
  ...
  if ($self->{SEARCH_FIELDS} =~ /m\./xm || $WHERE =~ /m\./xm) {
  ...
  my $sql = <<"SQL";
SELECT mc.id, $self->{SEARCH_FIELDS}
     mc.name,
     mc.inner_chapter,
     mc.responsible,
     mc.autoclose,
...
sub admin_change {
  ...
  my @chapters = split(/,\s/x, $attr->{IDS});
  my @MULTI_QUERY = ();
...
sub message_reply_del {
  ...
  if ($attr->{MAIN_MSG}) {
    my @id_arr = split(/,/x, $attr->{MAIN_MSG});
...
SQL
    ...
    $self->query($sql, 'do', { Bind => \@WHERE_VALUES });
  }
  elsif ($attr->{ID}) {
...
SQL
  ...
  my $sql = <<"SQL";
    DELETE FROM msgs_reply WHERE $WHERE
SQL
...
sub messages_reply_list {
  ...
  my $WHERE = $self->search_former($attr, [
    ...
    [ 'CONTACT_ID',    'INT',  'mr.contact_id',         1 ],
    [ 'CONTACT_COMMENTS', 'INT',  'uc.comments', 'uc.comments AS contact_comments',         1 ],
  ],
    {
      WHERE_RULES => \@WHERE_RULES,
      WHERE       => 1,
  ...
  if ($self->{SEARCH_FIELDS} =~ /uc\./ || $WHERE =~ /uc\./) {
    $EXT_TABLES = "\nLEFT JOIN users_contacts uc ON (uc.id = mr.contact_id) ";
  }
...
sub message_reply_info {
  ...
  $self->query('SELECT * FROM msgs_reply WHERE id = ? ',
    undef,
    { INFO => 1,
      Bind => [ $id ] }
  );
...
sub attachments_list {
  ...
  my $sql = <<"SQL";
  SELECT $self->{SEARCH_FIELDS} ma.id
...
sub attachment_info {
  ...
  my $sql = <<"SQL";
SELECT id AS attachment_id, filename,
       content_type,
       content_size,
       message_type,
       message_id,
...
SQL
  ...
  delete($self->{errno}) if ($self->{errno} && $self->{errno} == 2);
...
sub survey_subjects_list {
  ...
  my @search_params = (
    ...
    [ 'USED_SURVEY', 'INT', '(COUNT(DISTINCT mm.id) + COUNT(DISTINCT mr.id)) AS used_survey', 1 ],
  );
  ...
  if ($attr->{USED_SURVEY}) {
    $EXT_TABLE .= "\nLEFT JOIN msgs_messages mm ON ms.id = mm.survey_id";
    $EXT_TABLE .= "\nLEFT JOIN msgs_reply mr ON ms.id = mr.survey_id";
  }

  my $sql = <<"SQL";
SELECT ms.id, ms.name, $self->{SEARCH_FIELDS} ms.id AS survey_id
...
sub survey_subject_info {
  ...
  $self->query("SELECT *, id AS survey_id FROM msgs_survey_subjects WHERE id= ? ",
    undef, { INFO => 1, Bind => [ $id ] }
  );
...
sub survey_answer_list {
  ...
  my $WHERE = $self->search_former($attr, [
    [ 'QUESTION_ID',   'INT',  'ma.question_id',                       1 ],
    [ 'UID',           'INT',  'ma.uid',                               1 ],
    [ 'ANSWER',        'STR',  'ma.answer',                            1 ],
    [ 'COMMENTS',      'STR',  'ma.comments',                          1 ],
    [ 'DATE_TIME',     'DATE', 'ma.date_time',                         1 ],
    [ 'SURVEY_ID',     'INT',  'ma.survey_id',                         1 ],
    [ 'PARAMS',        'INT',  'mq.params',                            1 ],
    [ 'LOGIN',         'STR',  'u.id as login',                        1 ],
    [ 'COUNT_ANSWERS', 'INT',  'COUNT(ma.reply_id) AS count_answers',  1 ],
  ], { WHERE => 1 });

  my $EXT_TABLES = $self->{EXT_TABLES} || '';

  if ($self->{SEARCH_FIELDS} =~ /mq\./) {
  ...
  $self->query("SELECT $self->{SEARCH_FIELDS} ma.question_id
...
sub message_team_del {
  ...
  my @id_del_msgs = split(/,\s+/x, $id);

  my $ids = join(',', @id_del_msgs);
  $self->query("DELETE FROM `msgs_team_ticket` WHERE id IN ($ids)" );
...
sub respnosible_info_change {
  ...
  my $sql = <<'SQL';
SELECT mdt.id, mdt.responsible, mm.subject
FROM msgs_team_ticket AS mdt
...
sub subjects_list {
  ...
  $self->query("SELECT * FROM msgs_subjects $WHERE ORDER BY $SORT $DESC;",
    undef,
    $attr
  );
...
sub msgs_type_permits_list {
  ...
  $self->query("SELECT type, section, actions FROM msgs_type_permits $WHERE;",
    undef,
    { COLS_NAME => 1 }
  );
...
sub msgs_team_add {
  ...
  $self->query_add('msgs_teams', $attr);

  return $self;
}

#**********************************************************
=head2 msgs_team_change($attr)

=cut
#**********************************************************
sub msgs_team_change {
  my $self = shift;
  my ($attr) = @_;
...
sub msgs_team_del {
  ...
  $self->query_del('msgs_teams', $attr);

  return $self;
}

#**********************************************************
=head2 msgs_team_info()
...
sub msgs_team_info {
  ...
  $self->query("SELECT * FROM msgs_teams WHERE id = ? ", undef, { INFO => 1, Bind => [ $id ] });

  return $self;
}

#**********************************************************
=head2 msgs_teams_list($attr)
...
sub msgs_teams_list {
  ...
  my $WHERE = $self->search_former(
    ...
    [
      [ 'ID',            'INT',  'mt.id',         1 ],
      [ 'NAME',   'STR',  'mt.name',       1 ],
      [ 'RESPONSIBLE',        'INT',  'mt.responsible',            1 ],
      [ 'MESSAGES',     'INT',  'COUNT(mtm.message_id) AS messages',            1 ],
      [ 'DESCR',        'INT',  'mt.DESCR',            1 ],
    ],
    { WHERE => 1 }
  );

  $self->query("SELECT $self->{SEARCH_FIELDS} mt.id
    FROM msgs_teams mt
    LEFT JOIN msgs_team_messages mtm ON mtm.team_id = mt.id
    $WHERE
    GROUP BY mt.id", undef, $attr
  );
...
#**********************************************************
=head2 msgs_team_address_list($attr)

=cut
#**********************************************************
sub msgs_team_address_list {
  my $self = shift;
  my ($attr) = @_;
  ...
  my $WHERE = $self->search_former($attr, [
    [ 'TEAM_ID',       'INT', 'mta.team_id',       1 ],
    [ 'STREET_ID',     'INT', 'mta.street_id',     1 ],
    [ 'BUILD_ID',      'INT', 'mta.build_id',      1 ],
    [ 'DISTRICT_ID',   'INT', 'mta.district_id',   1 ]
  ], { WHERE => 1 });

  $self->query("SELECT mta.team_id, mta.street_id, mta.build_id, mta.district_id,
      SUBSTRING_INDEX(d.path, CONCAT('/', mta.district_id), 1) AS district_path, IF(mta.build_id, s.id, null) AS build_street
    FROM msgs_team_address mta
    LEFT JOIN builds b ON mta.build_id = b.id
    LEFT JOIN streets s ON b.street_id = s.id OR mta.street_id = s.id
    LEFT JOIN districts d ON d.id = s.district_id OR mta.district_id = d.id
    $WHERE
...
#**********************************************************
=head2 msgs_team_address_change($attr)

=cut
#**********************************************************
sub msgs_team_address_change {
  my $self = shift;
  my ($attr) = @_;

  return $self if !$attr->{TEAM_ID};

  my @MULTI_QUERY = ();

  if ($attr->{DISTRICT_IDS} && ref $attr->{DISTRICT_IDS} eq 'ARRAY') {
    for my $district_id (@{$attr->{DISTRICT_IDS}}) {
      push(@MULTI_QUERY, [$attr->{TEAM_ID}, $district_id, 0, 0]);
    }
  }

  if ($attr->{STREET_IDS} && ref $attr->{STREET_IDS} eq 'ARRAY') {
    for my $street_id (@{$attr->{STREET_IDS}}) {
      push(@MULTI_QUERY, [$attr->{TEAM_ID}, 0, $street_id, 0]);
    }
  }

  if ($attr->{BUILD_IDS} && ref $attr->{BUILD_IDS} eq 'ARRAY') {
    for my $build_id (@{$attr->{BUILD_IDS}}) {
      push(@MULTI_QUERY, [$attr->{TEAM_ID}, 0, 0, $build_id]);
    }
  }

  $self->msgs_team_address_del({ TEAM_ID => $attr->{TEAM_ID} });
  $self->query("INSERT INTO `msgs_team_address` (`team_id`, `district_id`, `street_id`, `build_id`) VALUES (?, ?, ?, ?);",
    undef, { MULTI_QUERY => \@MULTI_QUERY });
}

#**********************************************************
=head2 msgs_team_address_del($attr)

=cut
#**********************************************************
sub msgs_team_address_del {
  my $self = shift;
  my ($attr) = @_;

  $self->query_del('msgs_team_address', undef, { TEAM_ID => $attr->{TEAM_ID} });

  return $self;
}

#**********************************************************
=head2 msgs_team_members_change($attr)

=cut
#**********************************************************
sub msgs_team_members_change {
  my $self = shift;
  my ($attr) = @_;
  ...
  if ($attr->{AIDS} && ref $attr->{AIDS} eq 'ARRAY') {
    for my $aid (@{$attr->{AIDS}}) {
      push(@MULTI_QUERY, [$attr->{TEAM_ID}, $aid]);
    }
  }

  $self->msgs_team_members_del({ TEAM_ID => $attr->{TEAM_ID} });
  $self->query("INSERT INTO `msgs_team_members` (`team_id`, `aid`) VALUES (?, ?);",
    undef, { MULTI_QUERY => \@MULTI_QUERY });
}

#**********************************************************
=head2 msgs_team_members_del($attr)

=cut
#**********************************************************
sub msgs_team_members_del {
  my $self = shift;
  my ($attr) = @_;

  $self->query_del('msgs_team_members', undef, { TEAM_ID => $attr->{TEAM_ID} });

  return $self;
}

#**********************************************************
=head2 msgs_team_members_list($attr)

=cut
#**********************************************************
sub msgs_team_members_list {
  my $self = shift;
  my ($attr) = @_;
  ...
  $self->query("SELECT $self->{SEARCH_FIELDS} team_id
    FROM msgs_team_members
    $WHERE
    ORDER BY $SORT $DESC;", undef, $attr
  );
...
#**********************************************************
=head2 msgs_team_messages_change($attr)

=cut
#**********************************************************
sub msgs_team_messages_change {
  my $self = shift;
  my ($attr) = @_;

  return $self if !$attr->{MESSAGE_ID};

  $self->msgs_team_messages_del({ MESSAGE_ID => $attr->{MESSAGE_ID} });
  return $self if !$attr->{TEAM_ID};

  return $self->query_add('msgs_team_messages', $attr);
}

#**********************************************************
=head2 msgs_team_messages_del($attr)

=cut
#**********************************************************
sub msgs_team_messages_del {
  my $self = shift;
  my ($attr) = @_;

  $self->query_del('msgs_team_messages', undef, { MESSAGE_ID => $attr->{MESSAGE_ID} });

  return $self;
}

#**********************************************************
=head2 msgs_team_messages_info($attr)

=cut
#**********************************************************
sub msgs_team_messages_info {
  my $self = shift;
  my ($attr) = @_;

  return $self if !$attr->{MESSAGE_ID};

  $self->query("SELECT * FROM msgs_team_messages WHERE message_id = ? ", undef, { INFO => 1, Bind => [ $attr->{MESSAGE_ID} ] });
...
sub messages_search {
  ...
  my @WHERE_RULES = ();
  push @WHERE_RULES, "(m.subject LIKE '%$attr->{SEARCH_TEXT}%' OR m.message LIKE '%$attr->{SEARCH_TEXT}%' OR m.id = '$attr->{SEARCH_TEXT}' OR r.id IS NOT NULL)";
...
#**********************************************************
=head2 external_chats_info()

=cut
#**********************************************************
sub external_chats_info {
  my $self = shift;
  my ($id) = @_;

  $self->query("SELECT mec.*, u.uid, u.id AS login, mm.id AS message_id, mm.subject FROM `msgs_external_chats` mec
    LEFT JOIN `msgs_messages` mm ON (mm.external_chat_id = mec.id)
    LEFT JOIN `users` u ON (u.uid = mec.uid)
    WHERE mec.id = ? ", undef, { INFO => 1, Bind => [ $id ] });

  return $self;
}

#**********************************************************
=head2 external_chats_list()
...
=cut
#**********************************************************
sub external_chats_list {
  my ($self, $attr) = @_;

  my @search_params = (
    [ 'ID',         'INT',  'mec.id',                       1 ],
    [ 'CHAT_ID',    'STR',  'mec.chat_id',                  1 ],
    [ 'NAME',       'STR',  'mec.name',                     1 ],
    [ 'TYPE',       'STR',  'mec.type',                     1 ],
    [ 'UID',        'INT',  'mec.uid',                      1 ],
    [ 'LOGIN',      'STR',  'u.id', 'u.id AS login',        1 ],
    [ 'MESSAGE_ID', 'INT',  'mm.id', 'mm.id AS message_id', 1 ],
    [ 'DATE',       'DATE', 'mec.date',                     1 ]
  );
  ...
  my $sql = <<"SQL";
    SELECT $self->{SEARCH_FIELDS} mec.id
      FROM msgs_external_chats mec
      LEFT JOIN msgs_messages mm ON (mm.external_chat_id = mec.id)
      LEFT JOIN users u ON (u.uid = mec.uid)
      $EXT_TABLE
      $WHERE;
SQL
  ...
  $self->query("SELECT COUNT(*) AS total FROM msgs_external_chats mec
    LEFT JOIN msgs_messages mm ON (mm.external_chat_id = mec.id)
    LEFT JOIN users u ON (u.uid = mec.uid)
    $EXT_TABLE
    $WHERE;",
...
#**********************************************************
=head2 external_chats_add($attr)

=cut
#**********************************************************
sub external_chats_add {
  my $self = shift;
  my ($attr) = @_;

  $self->query_add('msgs_external_chats', { %{$attr}, DATE => 'NOW()' });

  return $self;
}

#**********************************************************
=head2 external_chats_del()

=cut
#**********************************************************
sub external_chats_del {
  my $self = shift;
  my ($attr) = @_;

  $self->query_del('msgs_external_chats', $attr);

  return $self;
}

#**********************************************************
=head2 external_chats_change()

=cut
#**********************************************************
sub external_chats_change {
  my $self = shift;
  my ($attr) = @_;
