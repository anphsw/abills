...
use Abills::Base qw(in_array convert ssh_cmd cmd _bp);
use Nas;
...
sub form_nas {
  ...
  if ($FORM{NAS_ID} && !$FORM{del}) {
    ...
    form_nas_search({ %FORM, STANDART => 1 });
  }
  ...
  $Nas->{NAME_SEL} = $html->form_main({
    CONTENT => $html->form_select(
      'NAS_ID',
      {
        ...
        SEL_LIST   => $Nas->list({ %LIST_PARAMS, NAS_NAME => '_SHOW', NAS_IP => '_SHOW', PAGE_ROWS => 60000 }),
        SEL_KEY    => 'id',
        SEL_VALUE  => 'nas_name,ip',
        AUTOSUBMIT => 'form'
      }
    ),
    HIDDEN  => {
...
sub form_nas_console_command {
  ...
  if ($Nas_->{NAS_TYPE} =~ /mpd|accel/xm || ($type eq 'telnet')) {
    ...
    elsif ($Mikrotik->has_access() ne 1){
...
sub form_nas_search {
  ...
  if (defined $FORM{NAS_SEARCH} && $FORM{NAS_SEARCH} == 0) {
    if ($FORM{UID}) {
      ...
      if ($users->{TOTAL} && $list->[0]->{build_id}) {
        ...
        my $nas_list = $Nas->list({
          FULL_ADDRESS => '_SHOW',
          %FORM,
          NAS_NAME     => $FORM{NAS_NAME} || '_SHOW',
          NAS_IP       => $FORM{NAS_IP} || '_SHOW',
          MAC          => $FORM{MAC} || '_SHOW',
          LOCATION_ID  => $list->[0]->{build_id},
        });

        my $nases_table = form_nas_search_nas_table($nas_list, $table_caption);
    ...
    print form_nas_search_nas_table($nas_list, '');
    return 1;
  }
    ...
    $table->addrow(
      ...
      $html->element(
        ...
        $line->{nas_name},
        {
          ...
          'data-value' => "NAS_ID::$line->{id}" . "#@#NAS_ID1::". ($line->{nas_name}||q{}) . "#@#NAS_IP::$line->{ip}"
        }
      ),
      $line->{ip},
      $line->{nas_type},
