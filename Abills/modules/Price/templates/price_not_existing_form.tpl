<form method='POST' action='$SELF_URL' class='form-horizontal'>
    <input type='hidden' name='E_MAIL_2' value='%E_MAIL%'>
    <input type='hidden' name='module' value='Price'>
    <div class='box box-primary'>
        <div class='box-header with-border'>
            <h3 class='box-title'>
                <div class='row'>
                    <div class='col-md-11 col-md-offset-1'>
                        <h2><span class='fa fa-fw fa-wrench'></span>_{SUPPORT}_</h2>
                    </div>
                    <div class='col-md-11 col-md-offset-1'><h4>_{PACKAGE_SUPPORT}_</h4></div>
                </div>
            </h3>
        </div>
        <div class="box-body">
            <div class="form-group">
                <label class="col-md-2 control-element">_{YOU_DID_NOT_HAVE_A_FORM}_.</label>
                <div class='col-md-2'>
                    <button class='btn btn-block btn-primary' type='submit' name='CREATE_NEW'
                            value='Создать новую'>_{CREATE_NEW}_
                    </button>
                </div>
            </div>
        </div>
    </div>
</form>