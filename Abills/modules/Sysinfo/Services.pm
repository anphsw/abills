...
use Abills::Base qw/startup_files _bp/;
...
sub sysinfo_show_permissions_grant_tip{
  ...
  if ( !$sudoers_path ) {
    ...
    $html->message( 'info', $lang{TIP},
      "$lang{EXECUTE} <pre>echo \'$sudoers_string\' >> $sudoers_path/abills_sudoers</pre> $lang{AS} <strong>root</strong> $lang{TO_GRANT_PERMISSIONS}" );
  }
  else {
    $html->message( 'err', $lang{ERROR},
      $html->button( "$lang{YOU_SHOULD_DEFINE} WEB_SERVER_USER $lang{IN} /usr/abills/Abills/programs",
...
sub _sysinfo_restart_service {
  ...
  if (!$sudo_name){
    $html->message('err', "$lang{ERROR}", "$lang{YOU_SHOULD_DEFINE} 'SUDO' $lang{IN} Abills/programs");
    return -1;
  }
  elsif(! -f $sudo_name){
    $html->message('err', "$lang{ERROR}", "$lang{ERR_WRONG_DATA}: 'SUDO' $lang{IN} Abills/programs"
