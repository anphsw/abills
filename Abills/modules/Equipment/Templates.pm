...
sub equipment_tmpl_edit {
  ...
  if ($tmpl) {
    ...
    if ($buttons) {
      my $model_select = $html->form_select('ID',
        {
    ...
    if (ref $values eq 'ARRAY') {
      ...
      foreach my $var (@$values) {
        ...
        $table->addrow(
          @arr,
          $html->button('', "index=$index$pages_qs&PARAM=$FORM{PARAM}&DEL=$i",
            {
...
#**********************************************************
=head2 equipment_stats_edit($attr)
...
sub equipment_stats_edit {
  ...
  if ($FORM{DEL}) {
    ...
    if ($FORM{TOTAL} && $FORM{TOTAL} == 1) {
      Equipment::Graph::del_graph_data({
        NAS_ID => $FORM{NAS_ID},
        PORT   => $FORM{PORT},
        TYPE   => $FORM{TYPE}
      });
...
sub equipment_snmp_vlan_data {
  ...
  if ($info) {
    ...
    foreach my $key (sort {$a <=> $b} keys %$vars) {
      $table->addrow($html->b($key), $html->b($vars->{$key}->[0]), join(", ", @{$vars->{$key}->[1]}), join(", ", @{$vars->{$key}->[2]}));
    }
...
#**********************************************************
=head2 equipment_snmp_port_data($attr)
    ...
    if ($tmpl && $info->[0]->{result}) {
      ...
      foreach my $key (sort {$a <=> $b} keys %tmphash) {
        $table->addrow(
          $key,
          @{$tmphash{$key}},
          $html->button("$lang{CABLE_TEST}: $lang{PORT} $key", "index=$port_index&NAS_ID=$attr->{NAS_ID}&test=$key",
            {
              ICON  => 'fa fa-eye',
              title => "$lang{INFO}Port $key"
            }
          )
            . $html->button("$lang{STATS}: $lang{PORT} $key", "index=$stats_index&NAS_ID=$attr->{NAS_ID}&PORT=$key",
            {
...
sub equipment_snmp_stats {
  ...
  my $PORT_SEL = $html->form_select('PORT', {
    SELECTED  => $FORM{PORT},
  ...
  print $html->form_main({
    CONTENT => "$lang{PERIOD}: $lang{FROM} &nbsp" . $html->form_datetimepicker('FROM_DATE') .
      "&nbsp $lang{TO} &nbsp" . $html->form_datetimepicker('TO_DATE') .
  ...
  foreach my $in (sort {$a <=> $b} keys %ind) {
    ...
    print $html->make_charts({ DATA => \@data, XKEYS => \@larr, LABELS => \@larr, GRAPH_ID => $in, UNITS => 'Mb/s', HEADER => "$lang{PORT} $in" });
  }
...
sub equipment_obj {
  ...
  if (!$FORM{ID}) {
    ...
    foreach my $oid (keys(%SNMP::MIB)) {
      if ($SNMP::MIB{$oid}{'objectID'} =~ /\.1\.3\.6\.1\.4\.1\./xm) {
...
=cut
    ...
    if ($buttons) {
      my $obj_select = $html->form_select('ID', {
        SELECTED       => $attr->{ID} || $FORM{ID},
...
sub mibs_browser {
  ...
  if (!$FORM{IP}) {
    my $obj_select = $html->form_select('IP', {
      SELECTED       => $FORM{IP},
      SEL_LIST       => $Equipment->obj_list(
        {
          IP           => '_SHOW',
