...
sub _gcom_get_ports {
  ...
  if ($attr->{MODEL_NAME} =~ 'EL5610') {
    foreach my $key (sort keys %{$ports_info}) {
      if ($ports_info->{$key}{PORT_NAME} && $ports_info->{$key}{PORT_NAME} =~ /^p((\d+)\/(\d+))$/xm) {
    ...
    foreach my $key (sort keys %{$ports_info}) {
      if ($ports_info->{$key}{PORT_NAME} && $ports_info->{$key}{PORT_NAME} =~ /^g((\d+)\/(\d+))$/xm) {
        $ports_info->{$key}{BRANCH} = $1;
  ...
  foreach my $oid_name (keys %{$snmp}) {
    ...
    if ($snmp->{$oid_name}->{OIDS}) {
      ...
      foreach my $line (@$result) {
        ...
        my ($port_index, $onu_index) = $line =~ /(\d+\.\d+)\.(\d+)/xm;
