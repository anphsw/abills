...
sub _vsolution_get_ports {
  ...
  foreach my $key (keys %{$ports_info}) {
    if ($ports_info->{$key}{PORT_TYPE} && $ports_info->{$key}{PORT_TYPE} =~ /^1$/xm
      && $ports_info->{$key}{PORT_NAME} =~ /(.PON)(\d+\/\d+)$/xm) {
      my $type = lc($1);
...
sub _vsolution_get_fdb {
  ...
  my $snmp_template = get_vendors_oids($attr->{SNMP_TPL} || 'vsolution.snmp', $attr);
