...
sub _huawei_get_ports {
  ...
  foreach my $key (keys %{$ports_info}) {
    if ($ports_info->{$key}{PORT_TYPE} && $ports_info->{$key}{PORT_TYPE} =~ /^1|250$/xm && $ports_info->{$key}{PORT_NAME} =~ /(.PON)/xm) {
      my $type = lc($1);
...
sub _huawei_unregister {
  ...
  my %unreg_info = (
    2  => 'mac_serial',
    3  => 'info',
    #    4  => 'x4', #  hwGponDeviceAutoFindOntInfoOntVendorId
    #    5  => 'x5', #  hwGponDeviceAutoFindOntInfoOntVer
    6  => 'register',
    #    7  => 'x7',
    #    8  => 'x8',
    9  => 'vendor',
    10 => 'firnware',
    11 => 'version',
    12 => 'nni_type', #hwGponDeviceAutoFindOntInfoOntNniType
  );
  ...
  foreach my $pon_type (@types) {
    ...
    if($pon_type eq 'epon') {
      ...
      foreach my $line (@$unreg_result) {
        ...
        my ($type, $snmp_port_id, $onu_num) = split(/\./x, $id || q{});
        ...
        my $unreg_id = $snmp_port_id . '.' . $onu_num;

        $unregister_info{$unreg_id}->{$unreg_info{$type}} = $value;
        $unregister_info{$unreg_id}->{'branch'}     = _huawei_decode_port($snmp_port_id);
        $unregister_info{$unreg_id}->{'branch_num'} = $snmp_port_id;
        $unregister_info{$unreg_id}->{'pon_type'}   = $pon_type;
      }

      push @unregister, values %unregister_info;
    }
    else {
      my $unregister_gpon = _unregister_gpon($attr);

      if ($#{$unregister_gpon} > -1) {
        push @unregister, @$unregister_gpon;
      }
    }
  }
...
sub _unregister_gpon {
  ...
  my %unreg_info = (
    2  => 'mac_serial',
    3  => 'info',
    4  => 'ontvendorid', #  hwGponDeviceAutoFindOntInfoOntVendorId
    5  => 'version', #  hwGponDeviceAutoFindOntInfoOntVer
    6  => 'firmware', # OntMainSoftVer
    7  => 'equipment_id', #Equipment_id
    #    8  => 'x8',
    9  => 'vendor',
    #10 => 'firnware', #  hwGponDeviceAutoFindOntInfoCheckcode
    #11 => 'version',
    12 => 'nni_type', #hwGponDeviceAutoFindOntInfoOntNniType
  );

  $snmp->{unregister}->{UNREGISTER}->{OIDS} = '1.3.6.1.4.1.2011.6.128.1.1.2.48.1';
  ...
  foreach my $line (@$unreg_result) {
    ...
    print "$line <br>" if ($attr->{DEBUG});

    my ($id, $value) = split(/:/x, $line || q{}, 2);
    my ($oid_id, $snmp_port_id, $onu_num) = split(/\./x, $id || q{});
    my $unreg_id = $snmp_port_id . '.' . $onu_num;

    next if (!$unreg_info{$oid_id});

    if ($unreg_info{$oid_id} eq 'mac_serial') {
      $unregister_info{$unreg_id}->{$unreg_info{$oid_id}} = bin2hex($value);
    }
    elsif ($oid_id == 12) {
      $unregister_info{$unreg_id}->{$unreg_info{$oid_id}} = ($nni_type{$value}) ? $nni_type{$value} : $value;
    }
    elsif ($oid_id == 5 || $oid_id == 6) {
      $value =~ s/Ww+//xg;
      $unregister_info{$unreg_id}->{$unreg_info{$oid_id}} = $value;
    }
    elsif ($oid_id == 9) {
      if ($unregister_info{$unreg_id}->{'mac_serial'}) {
        $unregister_info{$unreg_id}->{'vendor'} = bin2hex($unregister_info{$unreg_id}->{'mac_serial'});
        $unregister_info{$unreg_id}->{'vendor'} =~ s/[A-F0-9]{8}$//xg;
        $unregister_info{$unreg_id}->{'vendor'} = pack 'H*', $unregister_info{$unreg_id}->{'vendor'};
      }

      if ($unregister_info{$unreg_id}->{'equipment_id'}) {
        $unregister_info{$unreg_id}->{'equipment_id'} =~ s/[^a-zA-Z0-9-_]//xg;

        if ($unregister_info{$unreg_id}->{'vendor'} eq 'HWTC') {
          $unregister_info{$unreg_id}->{'equipment_id'} = "HG8$unregister_info{$unreg_id}->{'equipment_id'}" if ($unregister_info{$unreg_id}->{'equipment_id'} =~ /^\d{3}/xm);
        }
        elsif ($unregister_info{$unreg_id}->{'vendor'} eq 'ALCL') {
          $unregister_info{$unreg_id}->{'equipment_id'} =~ s/_//xg;
        }
      }
    }
    else {
    ...
    $unregister_info{$unreg_id}->{'branch_num'} = $snmp_port_id;
    $unregister_info{$unreg_id}->{'pon_type'}   = $pon_type;
    $unregister_info{$unreg_id}->{'type'}       = $pon_type;
  }
...
sub _huawei_unregister_form {
  ...
  $attr->{snmp}=$snmp;
  my ($line_profiles,  $srv_profiles) = _get_profiles($attr);
  ...
  foreach my $line (@$profile_snmp_vlans) {
    my ($profile_name, $vlan)=split(/:/x, $line);

    $profile_name =~ s/^(\d+)\.//xm;
    $profile_name =~ s/\.$vlan\.\d+$//xg;
    if ($debug > 1) {
...
sub _huawei_prase_line_profile {
  ...
  if ($pon_type eq 'gpon') {
    ...
    my $huawei_profile_name = _huawei_make_profile_name($attr->{LINE_PROFILE});
    @$data = map { $_ =~ /^$huawei_profile_name\.([\d\.]+:[\d]+)$/xm } @$data;
  }
...
#**********************************************************
=head2 _huawei_convert_distance($distance);
...
sub _huawei_convert_cpu_usage {
  ...
  my ($oid, $value) = split(/:/x, $data, 2);
  my ($frame, $slot) = split(/\./x, $oid, 2);
...
sub huawei_parse_profile_name {
  ...
  $name = pack('(C)*', split(/\./x, $name || q{}));
  $name =~ s/[\r\n]+//x;
  return $name;
}

#**********************************************************
=head2 _huawei_make_profile_name
...
sub _huawei_get_fdb {
  ...
  if (_huawei_telnet_open($attr)) {
    ...
    my $fdb_expr = q{^.\s+(\d+|-).+(epon|gpon|eth)+\s+([a-f0-9]{2})([a-f0-9]{2})\-([a-f0-9]{2})([a-f0-9]{2})\-([a-f0-9]{2})([a-f0-9]{2})\s+(anti-spf|dynamic|static)\s+(\d+)+\s\/(\d+)\W+(\d+).*\s+(\S+)\s+(\S+)\s+.*\s+(\d+)};
    foreach my $line (@list) {
      #print "$line ||| <br>";
      ...
      if ($line =~ m/$fdb_expr/x) {
        ...
        $hash{$key}{2} = _huawei_encode_port($frame, $slot, $port, $port_type) . (($onu_id =~ /\d+/xm) ? ".$onu_id" : "");
        $hash{$key}{4} = $van_id . (($srv_port =~ /\d+/xm) ? " ( SERVICE_PORT: $srv_port )" : "");
        $hash{$key}{5} = $port_types->{ $port_type } . $frame . '/' . $slot . '/' . $port . (($onu_id =~ /\d+/xm) ? ":$onu_id" : "");
      }
    }
  }
...
#**********************************************************
=head2 _get_profiles($attr) - get profiles
...
sub _get_profiles {
  ...
  return \@line_profiles, \@srv_profiles;
}
