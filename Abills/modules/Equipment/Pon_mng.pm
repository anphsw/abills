...
sub equipment_pon_init {
  ...
  my %vendor_list = (
    ...
    HUAWEI       => '_huawei',
    BDCOM        => '_bdcom',
    'V-SOLUTION' => '_vsolution',
    CDATA        => '_cdata',
    RAISECOM     => '_raisecom',
    SMARTFIBER   => '_smartfiber',
    GCOM         => '_gcom'
  );

  foreach my $vendor ( keys %vendor_list ) {
    if ($vendor_name =~ /$vendor/xmi) {
      $nas_type = $vendor_list{$vendor};
      $vendor =~ s/\-//gx;
      my $vendor_module = 'Equipment/Vendors/'. ucfirst(lc($vendor)) . '.pm';
...
#********************************************************
=head2 get_vendors_oids($type, $attr) - Get oid tpl
...
sub equipment_pon {
  ...
  $attr->{PON_TYPE}=$FORM{PON_TYPE} || $FORM{ONU_TYPE} || '';
  ...
  my $snmp = &{\&{$nas_type}}({ TYPE => $attr->{PON_TYPE}, MODEL => $attr->{NAS_INFO}{MODEL_NAME} });
  ...
  if ($FORM{unregister_list}) {
    ...
    equipment_onu_reset({ %$attr, %FORM, SNMP => $snmp, SNMP_COMMUNITY => $SNMP_COMMUNITY });
  }
  elsif (($FORM{disable_catv_port} || $FORM{enable_catv_port}) && $FORM{ONU}) {
    equipment_onu_catv({ %FORM, %$attr, SNMP => $snmp, SNMP_COMMUNITY => $SNMP_COMMUNITY });
  }
  elsif (($FORM{disable_eth_port} || $FORM{enable_eth_port}) && $FORM{ONU}) {
    equipment_onu_eth_port({ %FORM, SNMP => $snmp, SNMP_COMMUNITY => $SNMP_COMMUNITY });
  }

  if ($FORM{del_onu} && $FORM{COMMENTS}) {
    ...
    pon_onu_state($FORM{ONU}, {
      ...
      ONU_SNMP_ID => $FORM{info_pon_onu} || $FORM{ONU},
    });

    return 1;
  }
  elsif ($FORM{graph_onu}) {
    equipment_pon_onu_graph({ ONU_ID => $FORM{graph_onu}, snmp => $snmp });
  }
  elsif ($FORM{reg_onu}) {
    ...
    print $html->button($lang{UNREGISTER} . ' ' . ($#{$macs} + 1),
      "index=$index&visual=$FORM{visual}&NAS_ID=$nas_id&PON_TYPE=$attr->{PON_TYPE}&unregister_list=1",
...
#********************************************************
=head2 equipment_onu_catv($attr) - Show unregister OLN ONU
...
=cut
#********************************************************
sub equipment_onu_catv {
  my ($attr) = @_;

  my $snmp = $attr->{SNMP};
  my $SNMP_COMMUNITY = $attr->{SNMP_COMMUNITY};

  my $catv_port_id = $attr->{disable_catv_port} || $attr->{enable_catv_port};

  my $result = equipment_tv_port({
    NAS_INFO       => $attr->{NAS_INFO},
    snmp           => $snmp,
    ONU_SNMP_ID    => $attr->{ONU},
    SNMP_COMMUNITY => $SNMP_COMMUNITY,
    CATV_PORT_ID   => $catv_port_id,
    DISABLE_PORT   => $attr->{disable_catv_port},
    ENABLE_PORT    => $attr->{enable_catv_port}
  });

  if ($result) {
    if ($attr->{disable_catv_port}) {
...
#********************************************************
=head2 equipment_onu_eth_port($attr) - Show unregister OLN ONU
...
sub equipment_onu_eth_port {
  ...
  if (!$snmp->{eth_port_manage}) {
    ...
    my $port_id = $attr->{disable_eth_port} || $attr->{enable_eth_port};

    my $set_value;
    if ($attr->{disable_eth_port}) {
      $set_value = $snmp->{eth_port_manage}->{DISABLE_VALUE};
    }
    elsif ($attr->{enable_eth_port}) {
...
#********************************************************
=head2 equipment_onu_reset($attr) - Show unregister OLN ONU
...
sub equipment_onu_reset {
  ...
  if ($snmp->{reset}->{RESET_FN} && defined(&{$snmp->{reset}->{RESET_FN}})) {
  ...
  elsif ($snmp->{reset}->{SEPARATE}) {
    #specific to Vsolution #XXX move to RESET_FN?
    my ($pon_id, $onu_id) = $attr->{onuReset} =~ /(\d+)\.(\d+)/xm;
    ...
    $reset_result = snmp_set({
      SNMP_COMMUNITY => $SNMP_COMMUNITY,
      VERSION        => $attr->{VERSION},
      OID            => [ $snmp->{reset}->{OIDS} . '.' . $attr->{onuReset}, $snmp->{reset}->{VALUE_TYPE} || "integer", $reset_value ],
      DEBUG          => $attr->{DEBUG}
    });
  }
...
#********************************************************
=head2 equipment_onu_list($attr) - Show unregister OLN ONU
...
sub equipment_onu_list {
  ...
  foreach my $line (@$port_list) {
    ...
    if ($attr->{PON_TYPE} && $attr->{PON_TYPE} eq $line->{pon_type}) {
  ...
  if (!$FORM{SERVICE_PORTS} && !$FORM{LINE_PROFILES}) {
    $info{PON_TYPE} = $html->form_select('PON_TYPE',
      {
        SELECTED    => $attr->{PON_TYPE},
        SEL_HASH    => $pon_types,
    ...
    $info{OLT_PORT} = $html->form_select('OLT_PORT',
      {
        SELECTED    => $FORM{OLT_PORT},
        SEL_HASH    => $olt_ports,
        SEL_OPTIONS => { '' => $lang{SELECT_PORT} },
        EX_PARAMS   => " data-auto-submit='index=$index&visual=$FORM{visual}&NAS_ID=$nas_id&PON_TYPE=$attr->{PON_TYPE}' ",
        NO_ID       => 1
      });
  }
  ...
  if (defined(&$unregister_fn)) {
    if (!$Equipment->{STATUS}) {
      $unregister_btn = $html->button("$lang{UNREGISTER} <span class='fa fa-spinner fa-spin'></span>",
        "index=$index&visual=4&NAS_ID=$nas_id&PON_TYPE=$attr->{PON_TYPE}&unregister_list=1",{
          ID     => 'unreg_btn',
  ...
  $LIST_PARAMS{PON_TYPE} = $attr->{PON_TYPE} || '';
  ...
  my ($table, $onu_list) = result_former({
    ...
    FUNCTION        => 'onu_list',
    DEFAULT_FIELDS  => 'ID,BRANCH,ONU_ID,MAC_SERIAL,STATUS,RX_POWER',
    ...
    TABLE           => {
      ...
      DATA_TABLE       => { lengthMenu => [ [ 25, 50, 1000 ], [ 25, 50, $lang{ALL} ] ] },
      ID               => 'EQUIPMENT_ONU',
      EXPORT           => 1,
    }
  });
  ...
  if (in_array('mac_behind_onu', \@cols) ||
    (in_array('external_system_link', \@cols) && $conf{EQUIPMENT_USER_LINK} && $conf{EQUIPMENT_USER_LINK} =~ m/%USER_MAC%/x)) {
  ...
  if (in_array('distance', \@cols) && $onu_list->[0]) {
    ...
    foreach my $pon_type (keys %$pon_types) {
      ...
      my $branch_snmp_id;
      if ($FORM{OLT_PORT} && $onu_list->[0]->{onu_snmp_id} =~ m/^(\d+)\.\d+/x) {
        #on some OLTs it's possible to query single branch
        $branch_snmp_id = $1;
        if ($attr->{VENDOR_NAME} eq 'GCOM' && $pon_type eq 'epon' && $onu_list->[0]->{onu_snmp_id} =~ m/^(\d+\.\d+)\.\d+/x) {
          $branch_snmp_id = $1;
        }
      }

      my $timeout = $attr->{NAS_INFO}->{SNMP_TIMEOUT} || $conf{EQUIPMENT_INFO_SNMP_TIMEOUT} || 0;
  ...
  if (in_array('onu_name', \@cols) && $onu_list->[0]) {
    my $onu_name;
    foreach my $pon_type (keys %$pon_types) {
      next if ($FORM{OLT_PORT} && $onu_list->[0]->{pon_type} ne $pon_type);
      next if (!$snmp->{$pon_type}->{main_onu_info}->{ONU_NAME}->{OIDS});

      my $branch_snmp_id;
      if ($FORM{OLT_PORT} && $onu_list->[0]->{onu_snmp_id} =~ m/^(\d+)\.\d+/xm) {
        $branch_snmp_id = $1;
      }
      ...
      my $result = snmp_get({
        OID            => $snmp->{$pon_type}->{main_onu_info}->{ONU_NAME}->{OIDS} . ((defined $branch_snmp_id) ? ".$branch_snmp_id" : ''),
        SNMP_COMMUNITY => $SNMP_COMMUNITY,
        VERSION        => $attr->{VERSION},
        WALK           => 1,
        TIMEOUT        => $timeout
      ...
      foreach my $line (@$result) {
        my ($snmp_id, $value) = split(/:/x, $line);
        $snmp_id = "$branch_snmp_id.$snmp_id" if ($branch_snmp_id);
        my $function = $snmp->{$pon_type}->{main_onu_info}->{ONU_NAME}->{PARSER};
        if ($function && defined(&{$function})) {
          ($value) = &{\&$function}($value);
        }
        $onu_name->{$pon_type}->{$snmp_id} = $value;
      }
    }

    foreach my $line (@$onu_list) {
      $line->{onu_name} = $onu_name->{$line->{pon_type}}->{$line->{onu_snmp_id}} || '--';
    }
  }
  ...
  foreach my $onu (@$onu_list) {
    ...
    for (my $i = 0; $i <= $#cols; $i++) {
      ...
      if (in_array($col_id, [ 'login', 'address_full', 'user_mac', 'fio', 'comments' ])) {
        push @row,  _onu_user_info({
          ONU_INFO   => $onu,
        ...
        push @row,  _onu_ext_link({
          ONU_INFO       => $onu,
          USED_PORTS     => $used_ports,
          COL_ID         => $col_id,
          SNMP           => $snmp,
          MAC_BEHIND_ONU => \%mac_behind_onu
        });
      }
      else {
    ...
    if (!$onu->{deleted}) {
      push @control_row, $html->button('', "index=$index" . $page_gs . "&onuReset="
        . $onu->{onu_snmp_id} . "&PON_TYPE=" . $onu->{pon_type},
    ...
    push @control_row, $html->button($lang{DEL},
      "NAS_ID=$FORM{NAS_ID}&index=" . get_function_index('equipment_info')
        . "&visual=$FORM{visual}&PON_TYPE=" . ($onu->{pon_type} || 0) . "&del_onu=" . ($onu->{id} || 0),
  ...
  for my $line (@$onu_list) {
    if (in_array($line->{onu_status}, [1, 2, 3, 5, 18])) {
      $total_on += 1;
    }
    else {
      $total_off += 1;
  ...
  my $result_row_former = result_row_former({
    table => $table,
    ROWS  => \@all_rows,
  });
  ...
  $Equipment->{VENDOR_NAME} = $Equipment->{NAME} || '';
  $Equipment->{MAIN_INFO} = $html->button('', "index=$index&NAS_ID=%NAS_ID%", { ADD_ICON => 'fa fa-columns', TITLE => $lang{INFO} });

  my $pon_onus_signal_info = $html->element('div', _equipment_panel_onu_status($Equipment), { class => 'modal-body' });
  my $epon_ports_info = _equipment_panel_branch_onu_status({ %{$Equipment}, PON_TYPE => 'epon' });
  my $gpon_ports_info = _equipment_panel_branch_onu_status({ %{$Equipment}, PON_TYPE => '!epon' });
  my $Nas = Nas->new($db, \%conf, $admin);
  $Nas->info({ NAS_ID => $nas_id });

  $html->tpl_show(_include('equipment_pon_ports_onus_all', 'Equipment'), {
    %{$Equipment},
    %{$Nas},
    %FORM,
    PON_ONUS_SIGNAL_INFO => $pon_onus_signal_info,
    EPON_PORTS_INFO      => $epon_ports_info,
    GPON_PORTS_INFO      => $gpon_ports_info,
    PON_ONU_TABLE        => $result_row_former,
  });
...
#********************************************************
=head2 _onu_ext_link($attr) - Show unregister OLN ONU
...
sub _onu_ext_link {
  ...
  my $value = q{};
  my $onu = $attr->{ONU_INFO};
  my $snmp = $attr->{SNMP};
  my $mac_behind_onu = $attr->{MAC_BEHIND_ONU};
  ...
  $link =~ s/%CPE_MAC%/$onu->{mac_serial}/xg;
  $link =~ s/%SW_PORT%/$onu->{branch}:$onu->{onu_id}/xg;
  $link =~ s/%USER_MAC%/$macs/xg;

  $value = $html->button($lang{EXTERNAL_SYSTEM_LINK}, '',
    {
      class      => 'btn btn-secondary',
...
#********************************************************
=head2 _onu_user_info($attr) - Show unregister OLN ONU
...
sub _onu_user_info {
  ...
  if ($col_id eq 'login') {
    ...
    foreach my $user_info (@{$used_ports->{$onu->{dhcp_port}}}) {
      $value .= $html->br() if ($value);
      if ($col_id eq 'address_full') {
        $value .= $user_info->{address_full} || "";
      }
      elsif ($col_id eq 'user_mac') {
        $value .= $user_info->{cid} || ""; #TODO color_mark code
      }
      elsif ($col_id eq 'fio') {
        $value .= $user_info->{fio} || "";
      }
...
sub equipment_register_onu_cmd {
  ...
  if (equipment_onu_add($result_code, $result, $nas_id, $port_list, $attr)) {
...
#********************************************************
=head2 equipment_onu_add($result_code, $result) - add onu info to DB
...
=cut
#********************************************************
sub equipment_onu_add {
  my ($result_code, $result, $nas_id, $port_list, $attr) = @_;

  if ($result_code) {
    $result =~ s/\n/ /xg;

    my %add2db = (
      'default' => 'ONU:\s+\d+\/\d+\/\d+\:(\d+)\s+ADDED', #Huawei
      'zte'     => 'ONU\s+ZTE:\s+(\d+)\/(\d+)\/(\d+)\:(\d+)\s+ADDED',
      'eltex'   => 'ONU\s+ELTEX:\s+(\d+)\/(\d+)\:(\d+)\s+ADDED',
      'dbcom'   => 'ONU\s+BDCOM:\s+(\d+)\/(\d+)\:(\d+)\s+.*\s+SNMP\s+ID\s+(\d+)\s+DHCP\s+PORT\s+([0-9a-f]{4})\s+ADDED'
    );

    foreach my $type ( keys %add2db) {
      my $type_expr = $add2db{$type};
      if ($result =~ /$type_expr/xm) {
        my $reg_function = 'equipment_register_onu_add_'. $type;
        my $onu_info = &{ \&$reg_function }($result, $nas_id, $port_list, $attr);
        equipment_onu2db($onu_info);
        return $onu_info;
      }
    }
  }
...
#********************************************************
=head2 equipment_onu2db($onu) - add onu info to DB
...
=cut
#********************************************************
sub equipment_onu2db {
  my ($onu) = @_;

  my $onu_list = $Equipment->onu_list({
    COLS_NAME => 1,
    OLT_PORT => $onu->{PORT_ID},
    ONU_SNMP_ID => $onu->{ONU_SNMP_ID}
  });
...
sub equipment_register_onu_add_default {
  ...
  my ($onu_id) = $result =~ /ONU:\s+\d+\/\d+\/\d+\:(\d+)\s+ADDED/xm;

  my %onu = (
    ONU_ID         => $onu_id || 0,
    NAS_ID         => $nas_id,
    ONU_DHCP_PORT  => $port_list->[0]->{BRANCH} . ':' . $onu_id,
    PORT_ID        => $port_list->[0]->{ID},
    ONU_MAC_SERIAL => $attr->{MAC_SERIAL},
    ONU_DESC       => $attr->{ONU_DESC},
    ONU_SNMP_ID    => $port_list->[0]->{SNMP_ID} . '.' . $onu_id,
    LINE_PROFILE   => $attr->{LINE_PROFILE},
    SRV_PROFILE    => $attr->{SRV_PROFILE},
    VLAN           => $attr->{VLAN_ID}
  );

  if ($result =~ /SVLAN:CVLAN\s+(\d+):(\d+)/xm) {
    $onu{SERVER_VLAN} = $1;
    $onu{VLAN} = $2;
  }
...
sub equipment_register_onu_add_zte {
  ...
  my ($shelf, $slot, $olt, $onu_id) = $result =~ /ONU\s+ZTE:\s+(\d+)\/(\d+)\/(\d+)\:(\d+)\s+ADDED/xm;
  ...
  my $encoded_onu = ($pon_type eq 'epon') ? zte_encode_onu(($model_name =~ /_V2$/xmi) ? 9 : 3, $shelf, $slot, $olt, $onu_id, $model_name) : 0;
  ...
  $onu{ONU_MAC_SERIAL} = ($attr->{MAC} && $attr->{MAC} =~ /^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/gxm) ? $attr->{MAC} : $attr->{SN};
...
sub equipment_register_onu_add_eltex {
  my ($result, $nas_id, $port_list, $attr) = @_;
  my($slot, $onu_id) = $result =~ /ONU\s+ELTEX:\s+(\d+)\/(\d+)\:(\d+)\s+ADDED/xm;
...
sub equipment_register_onu_add_bdcom {
  ...
  my $bdcom_expr = q{ONU\s+BDCOM:\s+(\d+)\/(\d+)\:(\d+)\s+.*\s+SNMP\s+ID\s+(\d+)\s+DHCP\s+PORT\s+([0-9a-f]{4})\s+ADDED};
  my %onu = ();

  (undef, undef, $onu{ONU_ID}, $onu{ONU_SNMP_ID}, $onu{ONU_DHCP_PORT}) = $result =~ /$bdcom_expr/xm;
...
sub equipment_delete_onu {
  ...
  my $nas_type = equipment_pon_init($attr);
  my $onu_id   = $FORM{del_onu};
  my $onu_info = $Equipment->onu_info($onu_id);
...
sub equipment_pon_onu {
  ...
  my $SNMP_COMMUNITY = $attr->{SNMP_COMMUNITY};
  my $snmp = &{\&{$nas_type}}({ TYPE => $FORM{PON_TYPE}, MODEL => $attr->{NAS_INFO}{MODEL_NAME} });
  ...
  my ($table, $onu_list) = result_former({
    ...
    FUNCTION       => 'onu_list',
    BASE_FIELDS    => 1,
    DEFAULT_FIELDS => 'ID,COMMENTS,MAC_SERIAL,STATUS,RX_POWER,LOGIN',
    SKIP_PAGES     => 1,
    EXT_TITLES     => \%show_cols,
    TABLE          => {
      width            => '100%',
      caption          => "PON ONU",
      qs               => $page_gs,
  ...
  foreach my $onu (@$onu_list) {
    ...
    my $btn_class = ($onu->{dhcp_port} && $used_ports->{$onu->{dhcp_port}}) ? 'btn-warning' : 'btn-success';

    my $data_value = ''
      . $search_result_input_name . '::' . $onu->{dhcp_port}
      . '#@#CPE_MAC::' . $onu->{mac_serial}
      . (($server_vlan) ? '#@#' . 'SERVER_VLAN::' . $server_vlan : q{})
      . '#@#' . ($vlan_for_port{$onu->{id}} ? "VLAN::$vlan_for_port{$onu->{id}}" : ($onu->{vlan} ? "VLAN::$onu->{vlan}" : '')) #XXX vlan_for_port - how is this supposed to work? it gets equipment_pon_ports.vlan_id when equipment_pon_ports.id = equipment_pon_onu.id, why?
    ;

    push @row, "<div value='$onu->{dhcp_port}' class='clickSearchResult'>"
      . "<button title='$onu->{dhcp_port}' class='btn $btn_class'"
      . " onclick=\"fillSearchResults('$search_result_input_name', '$data_value')\"  >"
      . uc($onu->{pon_type}) . " $onu->{branch}:$onu->{onu_id}</button>
        </div>";
...
=head2 pon_onu_state($id, $attr) - Get ONU info
  ...
  Arguments:
    ...
    $attr
      ...
      NAS_ID        - NAS_ID
      ONU_DHCP_PORT - ONU DHCP PORT = internet.port
      SIMPLE        - info as hash
      snmp
...
sub pon_onu_state {
  ...
  if (!$nas_type) {
    print "NO_PON_DEVICE_INIT\n";
    return [];
  }

  my %params = (ONU_SNMP_ID => $id);

  if ($attr->{ONU_DHCP_PORT}) {
    %params = (ONU_DHCP_PORT => $attr->{ONU_DHCP_PORT});
  }
  ...
  $onu_info->{ONU_SNMP_ID} //= $FORM{info_pon_onu} || $FORM{ONU} || q{};
  $id = $onu_info->{ONU_SNMP_ID};
  $onu_info->{PORT_ID} //= q{};
  $onu_info->{LOCATION_ID} = $onu_info->{BUILD_ID} || q{};
  $onu_info->{NAS_TYPE} //= $nas_type;
  $onu_info->{NAS_ID} //= $nas_id;

  my $pon_type = $onu_info->{PON_TYPE} || 'epon';
  my $snmp_info = &{\&{$nas_type}}({ TYPE => $pon_type, MODEL => $model_name });
  ...
  if ($attr->{SHOW_FIELDS}) {
    if($conf{EQUIPMENT_USER_LINK} && $conf{EQUIPMENT_USER_LINK} =~ m/%USER_MAC%/x) {
      $attr->{SHOW_FIELDS} .= ',MAC_BEHIND_ONU';
    }
  }

  my %onu_info = &{\&$func_get_onu_info}($id, {
    %$attr,
    SILENT    => 1,
    NAS_TYPE  => $nas_type,
    PON_TYPE  => $pon_type,
  ...
  if ($attr->{SHOW_FIELDS} && $attr->{SHOW_FIELDS} =~ /PICTURE/xmg ) {
  ...
  if ($attr->{SIMPLE}) {
    $onu_info{$id}{ONU_SNMP_ID} = $id;
    return \%onu_info;
  }

  @info = ( @{ _olt_manage($onu_info, $attr) }, @info );

  if (! $onu_info->{BRANCH}) {
    push @info, [ '',
      $html->element('span', $lang{ERR_NOT_REGISTRED}, { class => 'text-danger' })
    ];
  }

  my $billing_desc_ajax = << "[END]";
<script>
...
[END]
  ...
  push @info, [
    $html->b($lang{ONU_BILLING_DESC}) . $html->button('',
      ...
      { MESSAGE => $lang{CHANGE_ONU_DESC},
        ...
        AJAX  => 'onu_billing_desc_changed'
      })
      . $billing_desc_ajax,

    $html->element('span', $onu_info->{ONU_BILLING_DESC}, { id => 'ONU_BILLING_DESC' })
  ];
  ...
  if ($conf{EQUIPMENT_USER_LINK}) {
    ...
    my $link = $conf{EQUIPMENT_USER_LINK};
    my $sw_port = ($onu_info->{BRANCH} && $onu_info->{ONU_ID}) ? "$onu_info->{BRANCH}:$onu_info->{ONU_ID}" : '';
    $link =~ s/%CPE_MAC%/$onu_info->{MAC_SERIAL}/xg if ($onu_info->{MAC_SERIAL});
    $link =~ s/%SW_PORT%/$sw_port/xg;
    $link =~ s/%USER_MAC%/$macs/xg;
  ...
  push @info, @{port_result_former(\%onu_info, {
    ...
    NAS_ID       => $nas_id,
    DHCP_PORT    => $onu_info->{ONU_DHCP_PORT} || $attr->{ONU_DHCP_PORT},
    PON_TYPE     => $pon_type
  ...
  if ($function && defined(&{$function})) {
    my $service_ports = &{\&$function}({
      %{$attr},
      BRANCH => $onu_info->{BRANCH},
      ONU_ID => $onu_info->{ONU_ID}
    });

    foreach my $s_port (@$service_ports) {
      push @info, [ $s_port->[0], $s_port->[1] ];
    }
  }
  ...
  if (in_array('Maps', \@MODULES)) {
    equipment_pon_map({ LOCATION_ID => $onu_info->{LOCATION_ID} || '' });
  }

  #Extra onu info
  if ($attr->{snmp} && $attr->{snmp}->{onu_info}) {
    _extra_onu_info({ %$attr, ONU_SNMP_ID => $id, SILENT => 1 });
  }
...
sub _extra_onu_info {
  ...
  my %info_oids = ();
  my $snmp = $attr->{snmp};
  my $id = $attr->{ONU_SNMP_ID} || 0;

  foreach my $oid_name (keys %{$snmp->{onu_info}}) {
  ...
  foreach my $oid_name (@cols) {
    if (!$snmp->{onu_info}->{ $info_oids{$oid_name} }) {
      next;
    }
    my $oid = $snmp->{onu_info}->{ $info_oids{$oid_name} } . '.' . $id;
    ...
    foreach my $line (@{$value_arr}) {
      my ($port_id, $value) = split(/:/x, $line, 2);
  ...
  my $ports_arr = snmp_get({
    %{$attr},
    WALK => 1,
    OID  => $snmp->{onu_info}->{ONU_PORTS_STATUS} .'.'. $id
...
sub _onu_tr_69 {
  ...
  my $tr_069_data = main::tr_069_get_data({
    QUERY      => {
      'InternetGatewayDevice.DeviceInfo.SerialNumber' => $onu_info->{MAC_SERIAL},
      #'InternetGatewayDevice.ManagementServer.Username' => $attr->{NAS_NAME}
    },
    PROJECTION => [ '_id' ], DEBUG => ($FORM{DEBUG} || 0)
  });
  if (!$tr_069_data->[0]->{_id} && $onu_info->{MAC_SERIAL}) {
    my $prefix_hex = substr($onu_info->{MAC_SERIAL}, 0, 8);
    my $suffix     = substr($onu_info->{MAC_SERIAL}, 8);

    my $decoded_prefix = pack("H*", $prefix_hex);

    $onu_info->{VENDOR_SERIAL} = $decoded_prefix . $suffix;

    $tr_069_data = main::tr_069_get_data({
      QUERY      => {
        'InternetGatewayDevice.DeviceInfo.SerialNumber' => $onu_info->{VENDOR_SERIAL},
        #'InternetGatewayDevice.ManagementServer.Username' => $attr->{NAS_NAME}
      },
      PROJECTION => [ '_id' ], DEBUG => ($FORM{DEBUG} || 0)
    });
  }
  if ($tr_069_data->[0]->{_id}) {
    $tr_069_data->[0]->{_id} =~ s/%2D/\*\*252D/xg;
  }
  my $tr_069_button = ($tr_069_data->[0]->{_id}) ? $html->button('',
    "NAS_ID=$onu_info->{NAS_ID}&index=" . get_function_index('equipment_info')
      . "&visual=4&ONU=$onu_info->{onu_snmp_id}&info_pon_onu=" . ($onu_info->{id} || q{}) . "&PON_TYPE=$onu_info->{PON_TYPE}&tr_069_id=$tr_069_data->[0]->{_id}",
    { class => 'btn btn-sm btn-success', ICON => 'fa fa-edit', TITLE => "TR-069" }) : '';

  return $tr_069_button;
}

#********************************************************
=head2 _olt_manage($attr)
...
sub _olt_manage {
  ...
  my $nas_tooltip = q{};
  my $nas_id = $onu_info->{NAS_ID};
  my $pon_type = $onu_info->{PON_TYPE} || '';
  my $snmp_id = $onu_info->{ONU_SNMP_ID};
  my $nas_type = $onu_info->{NAS_TYPE};
  $onu_info->{BRANCH} //= q{};
  $onu_info->{ONU_ID} //= q{};

  my $tr_069_button = _onu_tr_69($onu_info);
  ...
  if ($nas_id) {
    ...
    $extra_nas_info{NAS_NAME} = $Nas_info->{NAS_NAME} || '';
    $extra_nas_info{NAS_IP} = $Nas_info->{NAS_IP} || '';
    $extra_nas_info{NAS_MAC} = $Nas_info->{MAC} || '';
    $extra_nas_info{NAS_TYPE} = $Nas_info->{NAS_TYPE} || 'PON';

    $nas_tooltip =
      $html->b($lang{NAME}) . ': ' . $extra_nas_info{NAS_NAME} . $html->br()
        . $html->b('IP') . ': ' . $extra_nas_info{NAS_IP} . $html->br()
        . $html->b('MAC') . ': ' . $extra_nas_info{NAS_MAC} . $html->br()
        . $html->b('INFO') . ': click to telnet' . $html->br();
  }

  my $qs = "NAS_ID=$nas_id&ONU=$snmp_id&PON_TYPE=$pon_type&visual=4&index=" . get_function_index('equipment_info')
    . "&info_pon_onu=". ($onu_info->{id} || q{});
  # my $qs = "NAS_ID=$nas_id&ONU=$snmp_id&PON_TYPE=$pon_type&visual=4&index=" . get_function_index('equipment_info')
  #   . "&info_pon_onu=$snmp_id";
  ...
  my @info = ([
    ...
    $html->button("$pon_type " . ($onu_info->{BRANCH} || q{}) . ((defined $onu_info->{ONU_ID}) ? ":$onu_info->{ONU_ID}" : q{}), 'test',
      { class      => 'btn btn-sm btn-secondary',
        ...
        ex_params  => "data-tooltip='$nas_tooltip' data-tooltip-position='left'",
        GLOBAL_URL => 'telnet://' . $extra_nas_info{NAS_IP} })

      . $html->button('', $qs . "&OLT_PORT=" . $onu_info->{PORT_ID} . "&LOCATION_ID=" . ($onu_info->{LOCATION_ID} || q{}),
      { class => 'btn btn-sm btn-success', ICON => 'fa fa-info-circle', TITLE => $lang{INFO} })
      . $tr_069_button
      . $html->button('', $qs . "&onuReset=$snmp_id&tr_069_id=" . ($FORM{tr_069_id} || q{}),
      { MESSAGE => "$lang{REBOOT} ONU: $onu_info->{BRANCH}:$onu_info->{ONU_ID}?", class => 'btn btn-sm btn-warning', ICON => 'fa fa-retweet', TITLE => $lang{REBOOT} . " ONU" })
      . $html->button('', $qs . "&del_onu=" . ($onu_info->{ID} || 0),
      { MESSAGE => "$lang{DEL} ONU: $onu_info->{BRANCH}:$onu_info->{ONU_ID}?", class => 'btn btn-sm btn-danger', ICON => 'fa fa-ban', TITLE => "$lang{DEL} ONU" })
      . (($get_onu_config_function && defined(&{$get_onu_config_function})) ?
      $html->button($lang{SHOW_ONU_CONFIG}, $qs . "&get_onu_config=1" . "&OLT_PORT=$onu_info->{PORT_ID}"
        . "&LOCATION_ID=" . $onu_info->{LOCATION_ID},
...
sub default_get_onu_info {
  my ($id, $attr) = @_;
  my ($nas_type, $pon_type, $snmp_info, $db_info) = @{$attr}{'NAS_TYPE', 'PON_TYPE', 'SNMP_INFO', 'DB_INFO'};

  my %onu_info = ();

  my @show_fields = ();
  if ($attr->{SHOW_FIELDS}) {
    @show_fields = split(/,\s?/x, $attr->{SHOW_FIELDS});
  }
  else {
    @show_fields = @ONU_FIELDS;
  }

  foreach my $field (@show_fields) {
    if($db_info->{$field}) {
      $onu_info{$id}{$field} = $db_info->{$field};
    }
  }
  ...
  foreach my $oid_name (@oid_names) {
    ...
    my $timeout = $snmp_info->{$oid_name}->{TIMEOUT} || $attr->{TIMEOUT};
...
sub equipment_pon_onu_graph {
  ...
  my $onu_id   = $attr->{ONU_ID} || $FORM{graph_onu};
  my $onu_info = $Equipment->onu_info($onu_id);
  ...
  if ($FORM{FROM_DATE} && $FORM{FROM_DATE} =~ m/(\d+)-(\d+)-(\d+)\s+(\d+):(\d+)/x) {
    $start_time = timelocal(0, $5, $4, $3, $2 - 1, $1);
  }
  if ($FORM{TO_DATE} && $FORM{TO_DATE} =~ m/(\d+)-(\d+)-(\d+)\s+(\d+):(\d+)/x) {
  ...
  foreach my $graph_type (sort keys %graph_hash) {
    ...
    if ($graph) {
      ...
      my $chart_title = ($lang{uc($graph_type)} || $graph_type) . ' NAS_ID: ' . $nas_id . ' ONU: '
        . ($onu_info->{BRANCH} || q{}) . ((defined $onu_info->{ONU_ID}) ? ":$onu_info->{ONU_ID}" : q{}) ;
...
#**********************************************************
=head2 pon_onu_convert_state($nas_type, $status, $pon_type, $attr)
...
sub pon_onu_convert_state {
  my (undef, $status, undef, $attr) = @_;

  if (!defined $status) {
    $status = $ONU_STATUS_TEXT_CODES{NOT_EXPECTED_STATUS};
  }

  my ($status_desc, $color) = split(/:/x, $ONU_STATUS_CODE_TO_TEXT{ $status });
...
=head2 equipment_tv_port() - Disable or enable TV Port on ONU
  ...
  Arguments:
    $attr
      ...
      ONU_TYPE|PON_TYPE - required if snmp is not set
...
sub equipment_tv_port {
  ...
  my $pon_type = $attr->{PON_TYPE} || $attr->{ONU_TYPE};
  ...
  if (!$snmp_info) {
    ...
    $snmp_info = &{\&{$nas_type}}({ TYPE => $pon_type, MODEL => $nas_info->{MODEL_NAME} });
  }
...
#**********************************************************
=head2 equipment_change_onu_desc_ajax($attr) - change description from billing to ONU via SNMP

  Arguments:
    $FORM{SNMP_COMMUNITY}  - SNMP COMMUNITY
    $FORM{COMMENTS}        - Description
    $FORM{PORT}
    $FORM{SNMP_TPL}
    $FORM{PON_TYPE}
...
=cut
#**********************************************************
sub equipment_change_onu_desc_ajax {
  my $error = 0;
  my $errmsg = '';

  my $snmp_tpl = $FORM{SNMP_TPL} || '';
  $snmp_tpl =~ s/\.snmp//xg;
  my $snmp = _get_snmp($snmp_tpl);
  my $pon_type = $FORM{PON_TYPE} || q{};

  my $oid = $snmp->{$pon_type}->{ONU_DESC}->{OIDS} || '';
  return 0 if (!$oid);

  $oid .= ".$FORM{PORT}" if ($FORM{PORT});

  snmp_set({
    SNMP_COMMUNITY => $FORM{SNMP_COMMUNITY},
    OID            => [ $oid, "string", "$FORM{COMMENTS}" ]
  });
...
sub _equipment_panel_onu_status {
  ...
  for my $onu (@$onus) {
    if (in_array($onu->{status}, [1, 2, 3, 5, 18])) {
      $total_online += 1;
    }
    else {
      $total_offline += 1;
...
sub _equipment_panel_branch_onu_status {
  ...
  foreach my $branch (@branch_list_sorted) {
    ...
    my $onu_reg_total = $onu_online + $onu_offline;

    my $progress_bar = $html->progress_bar({
      TEXT     => (defined($onu_reg_total) > 0) ? "$onu_reg_total/$ports_total" : 0,
      TOTAL    => $ports_total,
      COMPLETE => (defined($onu_reg_total) > 0) ? $onu_reg_total : 0,
      COLOR    => ' bg-success'
    });
  ...
  if ($attr->{PON_TYPE} eq 'epon'){
    $FORM{'EPON_BRANCH_QUANTITY'} = $branch_quantity;
  }
  else {
    $FORM{'GPON_BRANCH_QUANTITY'} = $branch_quantity
  }
...
#********************************************************
=head2 _get_snmp ($snmp_template) - Get snmp file and return hash_ref

  Arguments:
    $snmp_template - Snmp template name
...
sub _get_snmp {
  ...
  if ($file_content) {
    ...
    my $json = JSON->new();
    my $snmp = $json->allow_nonref->decode($file_content);
