...
#  foreach my $key (keys %{ $attr->{NAS_INFO} }) {
#    print "$key -  $attr->{NAS_INFO}->{$key}<br>";
#  }
...
#        }
        ...
        $unregister[$num -1]->{'branch'}=decode_onu($branch, { SHELF_INC => 1 });
...
#    12 => 'x12',
  ...
  foreach my $line ( @$unreg_result ) {
    ...
    $unregister[$num -1]->{'branch'}=$branch;
  }
