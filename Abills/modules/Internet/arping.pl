...
use Abills::Init qw/$db $admin %conf @MODULES/;
...
sub find_nas_to_make_arping {
  ...
  $Nas->query("SELECT id, name FROM ippools WHERE vlan=?", undef, { Bind => [ $lease_vlan ], COLS_NAME => 1 });
  ...
  $Nas->query("SELECT id FROM nas WHERE id IN (SELECT nas_id FROM nas_ippools WHERE pool_id=?) $by_nas_type;", undef,
