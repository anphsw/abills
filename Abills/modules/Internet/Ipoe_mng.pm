...
sub internet_ipoe_activate {
  ...
  if ($Internet->{STATUS} && $Internet->{STATUS} > 0 && !$conf{INTERNET_IPOE_NEGATIVE}) {
    ...
    if ($user->{UID}) {
      ...
      my ($status_text, undef) = split(/:/x, $service_status->{$Internet->{STATUS}});
  ...
  if (!$user->{UID} && $FORM{NAS_ID}) {
    ...
    my $ip_num = unpack("N", pack("C4", split(/\./x, $ip)));
...
sub internet_ipoe_change_status {
  ...
  if ($attr->{FRAMED_IP_ADDRESS} !~ m/^$IPV4$/x) {
  ...
  if (!$cmd) {
    ...
    $cmd =~ s/\%SPEED_IN/$speed_in/xg if ($speed_in > 0);
    $cmd =~ s/\%SPEED_OUT/$speed_out/xg if ($speed_out > 0);
    ...
    if ($attr->{NAS_IP_ADDRESS}) {
      ...
      ($ENV{NAS_MNG_IP}, undef, $ENV{NAS_MNG_PORT}) = split(/:/x, $attr->{NAS_MNG_IP_PORT}, 4);
      $ENV{NAS_MNG_PORT} ||= 22;
    }
  ...
  if ($conf{INTERNET_IPOE_FILTER} && ($STATUS ne 'ONLINE_ENABLE' || ($STATUS eq 'ONLINE_ENABLE' && $FILTER_ID ne ''))) {
    ...
    $cmd =~ s/\%STATUS/$STATUS/xg;
    $cmd =~ s/\%IP/$ip/xg;
    $cmd =~ s/\%ACTION/$STATUS/xg;
    $cmd =~ s/\%MASK/$netmask/xg;
    $cmd =~ s/\%LOGIN/$USER_NAME/xg;
...
sub ipoe_sessions {
  ...
  foreach my $k (sort %report) {
    ...
    if ($FORM{DATE}) {
      my (undef, $h) = split(/\s/x, $stats_array->[0]->{start} || $stats_array->[0]->{hours}, 2);
      $h++;
      ...
      if ($stats_array->[0]->{start} && $stats_array->[0]->{start} =~ m/(\d+)-(\d+)-(\d+)/x) {
        $num = $3;
      }
      elsif ($stats_array->[0]->{start} && $stats_array->[0]->{start} =~ m/(\d+)-(\d+)/x) {
  ...
  if ((!$FORM{EX_PARAMS} || $FORM{EX_PARAMS} ne 'USERS')
    ...
    if ($FORM{EX_PARAMS} && $FORM{EX_PARAMS} eq 'HOURS') {
      ...
      foreach my $k (sort %report) {
        ...
        #Chart
        my (undef, $h) = split(/\s+/x, $stats_array->[0]->{hours}, 2);
        $h++;
