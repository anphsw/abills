...
<script>
  ...
  function _equipment_user_info_generate_body(res) {
    ...
    full_list.forEach(function(field) {
      ...
      let div = jQuery('<div></div>').addClass('abills-checkbox-parent').append(input).append(label);
      checkboxes.push(div);
    });
    ...
    let submit_btn = jQuery('<input/>')
                      ...
                      .attr('value', '_{SAVE}_');
    jQuery('<div></div>').addClass('abills-form-main-buttons justify-content-between').append(default_btn).append(submit_btn).appendTo(form);
