package Viber::buttons::Services_and_account;
...
sub click {
  ...
  $message .= sprintf("*$self->{bot}->{lang}->{DEPOSIT}:* %.2f $money_currency", $user_info->{DEPOSIT});
  ...
  for my $module (sort keys %$user_services) {
    ...
    $message .= "— $module_name: \n";
    if (!scalar(@$services)) {
      $message .= "_$self->{bot}->{lang}->{NOT_ACTIVE}_\n";
    }
    ...
    foreach my $i (0..$#$services) {
      ...
      my $number = $i + 1;
      $message .= "$number) *$tp_name*: ". ($statuses->{$possible_status} || "") . "\n";

      if ($service->{holdup} && !$service->{holdup}{errstr} && keys %{$service->{holdup}}) {
        $message .= "*$self->{bot}->{lang}->{SERVICE_STOP_DATE} $service->{holdup}->{DATE_TO}*\n";
      ...
      if ($service->{schedule}) {
        ...
        $message .= "$self->{bot}{lang}{TO} *$future_tp_name* ";
        ...
        if ($service->{schedule}->{CAN_CANCEL} && $service->{schedule}->{CAN_CANCEL} eq 'true') {
          my $inline_button = {
            ...
            ActionType => 'reply',
            ActionBody => "fn:Services_and_account&delete_schedule&$module&$service->{schedule}->{SHEDULE_ID}",
            TextSize   => 'regular'
          };
      ...
      if ($module eq 'Internet') {
        $message .= "$self->{bot}->{lang}->{SPEED}: *$service->{in_speed}*\n" if ($service->{in_speed});
      }

      # Fee block
      if ($module eq 'Abon') {
        if ($service->{period} eq 'month') {
          $message .= "$self->{bot}{lang}{PRICE_MONTH}: *$service->{price}* $money_currency\n";
        }
        elsif ($service->{period} eq 'day') {
          $message .= "$self->{bot}{lang}{PRICE_DAY}: *$service->{price}*$money_currency\n";
        }
      }
      else {
        $message .= "$self->{bot}->{lang}->{PRICE_MONTH}: *$service->{month_fee} $money_currency*\n" if ($service->{month_fee});
        $message .= "$self->{bot}->{lang}->{PRICE_DAY}: *$service->{day_fee} $money_currency*\n" if ($service->{day_fee});
      }
    }
    $message .= "\n";
  }

  $message .= "$self->{bot}->{lang}->{SUM_MONTH}: *$total_sum_month $money_currency*\n";
  ...
  if ($can_cancel_holdup) {
    my $inline_button = {
      ...
      ActionType => 'reply',
      ActionBody => 'fn:Services_and_account&stop_holdup',
      TextSize   => 'regular'
    };
    push (@inline_keyboard, $inline_button);
  }

  if ($self->{conf}->{VIBER_EQUIPMENT_INFO}) {
    my $inline_button = {
  ...
  my $inline_button = {
    ActionType => 'reply',
    ActionBody => 'fn:Services_and_account&cancel',
...
sub equipment_info_bot {
  ...
  if ($onus->{error} || !keys %$onus) {
    ...
    $message .= "$self->{bot}->{lang}->{ALLOW_MAC}: *$onu_info->{cid}*\n";
  }
...
sub online_info_bot {
  ...
  if ($session) {
    ...
    $message .= "$self->{bot}{lang}{IP_ADDRESS}: *$session->{ip}*\n";
    $message .= "$self->{bot}{lang}{CID_ADDRESS}: *$session->{cid}*\n";
    $message .= "$self->{bot}{lang}{DURATION}: *$session->{duration}* $self->{bot}{lang}{SECONDS}\n";
    $message .= "$self->{bot}{lang}{RECV}: *$input_label*\n";
    $message .= "$self->{bot}{lang}{SENT}: *$output_label*\n";
  }
  else {
