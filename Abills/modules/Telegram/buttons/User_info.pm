package Telegram::buttons::User_info;
...
sub click {
  ...
  $message .= "\n";
  $message .= "$self->{bot}->{lang}->{LOGIN}: <tg-spoiler>$user_info->{LOGIN}</tg-spoiler>\n";
  ...
  for my $cell_phone (@{$user_pi->{CELL_PHONE}}) {
    $message .= "$self->{bot}->{lang}->{YOUR_PHONE_CELL}: <tg-spoiler>$cell_phone</tg-spoiler>\n";
  }

  for my $phone (@{$user_pi->{PHONE}}) {
    $message .= "$self->{bot}->{lang}->{YOUR_PHONE}: <tg-spoiler>$phone</tg-spoiler>\n";
  }

  for my $email (@{$user_pi->{EMAIL}}) {
    $message .= "$self->{bot}->{lang}->{EMAIL}: <tg-spoiler>$email</tg-spoiler>\n";
  }
