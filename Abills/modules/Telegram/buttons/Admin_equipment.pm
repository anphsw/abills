package Telegram::buttons::Admin_equipment;

use strict;
use warnings FATAL => 'all';
use Abills::Base qw(in_array int2ip);
...
sub click {
  ...
  $message .= "\n";
  my ($equipments_text) = $self->_equipments_list($page, \@equipment_buttons);
  $message .= $equipments_text;
...
sub equipment_ports {
  ...
  my ($equipments) = $self->{api}->fetch_api({
  ...
  return if $equipments->{total} != 1;
  my $equipment_info = $equipments->{list}->[0];
...
sub search {
  ...
  my ($equipment_result, $total) = $self->_equipment_search($page, $search, \@equipment_buttons);
  if ($equipment_result) {
    push @info, $equipment_result;
    $search_text .= "$self->{bot}{lang}{TELEGRAM_FOUND} <b>$self->{bot}{lang}{EQUIPMENT}: $total</b>\n";
    push @inline_keyboard, \@equipment_buttons;
  }

  my ($onu_result, $onu_total) = $self->_onu_search($page, $search, []);
  if ($onu_result) {
    push @info, $onu_result;
    $search_text .= "$self->{bot}{lang}{TELEGRAM_FOUND} <b>ONU: $onu_total)</b>\n\n";
    $total = $onu_total if $total < $onu_total;
  }
...
sub _equipment_search {
  ...
  my ($equipment_result, $total) = $self->_equipments_list($page, $buttons, {
    NAS_NAME   => "*$search*",
    MAC        => "*$search*",
    NAS_IP     => "*$search*",
    _MULTI_HIT => 1
...
sub _onu_search {
  ...
  my ($onu_result, $total) = $self->_onu_list($page, '_SHOW', {
    ONU_DESC      => "*$search*",
    MAC_SERIAL    => "*$search*",
...
sub _onu_list {
  ...
  $self->{last_page} = int($onus->{total} / 5) + ($onus->{total} % 5 == 0 ? 0 : 1) if $onus->{total} > 5;

  my $number = 1;
  foreach my $onu (@{$onus->{list}}) {
...
sub _equipments_list {
  ...
  $self->{last_page} = int($equipments->{total} / 5) + ($equipments->{total} % 5 == 0 ? 0 : 1) if $equipments->{total} > 5;

  my $number = 1;
  foreach my $equipment (@{$equipments->{list}}) {
  ...
  return ($message, $equipments->{total});
}
...
sub _pon_ports_list {
  ...
  $self->{last_page} = int($ports->{total} / 5) + ($ports->{total} % 5 == 0 ? 0 : 1) if $ports->{total} > 5;

  my @info = ();
  my $number = 1;
  foreach my $port (@{$ports->{list}}) {
    my $icon = $icons{"number_" . $number++} || '';
    $port->{onu_count} ||= 0;
