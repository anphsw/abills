package Telegram::buttons::Balance;
...
sub click {
  ...
  $message .= sprintf("$icons{active} $self->{bot}{lang}{YOUR_DEPOSIT}: <b>%.2f $money_currency</b>\n", $user_info->{DEPOSIT});
  $message .= "\n";

  if ($recommended_pay->{all_services_sum}) {
    $message .= sprintf("$self->{bot}{lang}{MONTH_PAID_SUM}: <b>%.2f $money_currency</b>\n", $recommended_pay->{all_services_sum});
  }
