...
sub click {
  ...
  if($conf_info->{conf}{"PAYSYS_CITY24_ACCOUNT_KEY" . "_$gid"} eq "CONTRACT_ID"){
    $account_key = $users_info->{CONTRACT_ID};
  }
  elsif($conf_info->{conf}{"PAYSYS_CITY24_ACCOUNT_KEY" . "_$gid"} eq "LOGIN"){
    $account_key = $users_info->{LOGIN};
  }

  my $amount = abs($users_info->{DEPOSIT});
  my $fast_pay = $conf_info->{conf}{"PAYSYS_CITY24_FAST_PAY" . "_$gid"};
  my $url_pay = "$fast_pay"  . "?paymethodid=6" . "&number=" .  "$account_key" . "&amount=" . "$amount";

  if (defined $conf_info->{conf}{"PAYSYS_CITY24_ACCOUNT_KEY" . "_$gid"}) {
