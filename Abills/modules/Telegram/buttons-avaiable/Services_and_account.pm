...
sub click {
  ...
  $message .= sprintf("На вашем счету: %.2f\n", $Users->{DEPOSIT});
  $message .= "\n";
  ...
  my @inline_keyboard = ();
  $message .= "Вами заказаны услуги:\n";

  foreach my $line (@$list) {
    $message .= "Интернет: <b>$line->{tp_name}</b>\n";
  ...
  if ($credit_warn) {
    if ($self->{conf}{user_credit_change} && $Users->{ALLOW_CREDIT}) {
      $message .= "Чтобы продолжить пользоваться услугами, необходимо пополнить счет, или активировать кредит.\n";
      my $inline_button = {
        text          => "Взять кредит",
      ...
      $message .= "Чтобы продолжить пользоваться услугами, необходимо пополнить счет.\n";
    }
  }
...
sub credit {
  ...
  my $credit_warning = "Кредит недоступен.";
  ...
  if ($month_changes) {
    ...
    if ($self->{admin}->{TOTAL} >= $month_changes) {
      $credit_avaiable = 0;
      $credit_warning = "Вы исчерпали максимальное колличество использований кредита.";
    }
  }
