...
<head>
  <title>$conf{WEB_TITLE} - Login</title>
    ...
    <script src=\"/wifi/js/jquery.cookie.js\"></script>
    ...
    \$(function() {
     ...
     function logon(username, pass, remember) {
        \$.ajax({
           type: 'POST',
             ...
             console.log(data);
               ...
               function(data) {
                 ...
                 if (clientState != data.clientState) {
                   if (remember == true) { 
                     \$.cookie('hotspot_username', username);
                     \$.cookie('hotspot_password', pass);
                   }
                   clientState = data.clientState;
                   clientStateOld = clientState
                   setTimeout( get_content, 2000);
                 }
                 else {
                   \$.cookie('hotspot_username', null);
                   \$.cookie('hotspot_password', null);
                   \$( \"#username\" ).val('');
                   \$( \"#password\" ).val('');
                 }
               });
     ...
     function get_status() {
       interval=setInterval(\"status()\",10000);
       status();
     }
...
if (defined $clientstate && $clientstate == 0) {
  ...
  <script type=\"text/javascript\">
    interval=setInterval(\"status()\",30000);
    ...
    \$('#login-form').submit(function(e){
      ...
      \$('#loading').modal('show');
       ...
       var remember = \$('#login-form').find('input[name=remember]').prop('checked');
       logon(username, pass, remember);
    });
