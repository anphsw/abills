...
=head1  Authors: Kodr, Anykey, ~AsmodeuS~
...
if ( $FORM{ap} ){
  ...
  # General controller case
    ...
    $html->message( 'err', 'ERROR',
      $Nas->{errno} .
      "UNKNOWN_NAS IP: $ENV{REMOTE_ADDR} NAS_MAC: $FORM{ap}" .
  ...
  print $html->header();
  $html->message( 'err', 'ERROR',
    "NOT DEFINED AP MAC UNKNOWN_NAS IP: $ENV{REMOTE_ADDR} NAS_MAC: " . ($FORM{ap} || '')
...
sub main{
  ...
  if ( $result == 2 ){
    ...
    if ( $index eq -1 ){
      $html->tpl_show( _include( 'unifi_unrecognized_mac', 'Unifi' ), {
          USERMAC => $usermac
        } );
      exit( 0 );
    }
...
sub get_status_update {
  ...
  if ( $mac ) {
    ...
    print "*/" if ($debug);
    print "Content-Type: application/javascript\n\n" if (!$debug);
#    _bp("Client data", $client_data , {TO_CONSOLE => 1});
    print << "JSON_UPDATE";
...
sub read_query_parameters{
  ...
  $return_url = urlencode( "$SELF_URL?operation_type=return&id=". ($usermac || '') .'&ap='. ($apmac || '') ."&username=%LOGIN%&password=%PASSWORD%" );
  ...
  if ( defined $operation_type ){
    ...
    if ( $operation_type eq "return" ){
      $usermac       = urldecode( $COOKIES{'hotspot_user_id'} );
      $apmac         = urldecode( $COOKIES{'hotspot_ap_mac'} );
      $userurldecode = urldecode( $COOKIES{'hotspot_user_url'} );
    }
...
sub print_no_mac_provided{
  ...
  $html->tpl_show( _include( 'unifi_no_mac_provided', 'Unifi' ),
      {
...
sub print_login_form{
  ...
  $html->tpl_show( _include( 'unifi_login_form', 'Unifi' ),
    {
...
#  print_java_script_variables();
  ...
  $html->tpl_show( _include( 'unifi_status_form', 'Unifi' ),
    {
      USERMAC     => $usermac,
...
#********************************************************************
sub print_header{
  $html->tpl_show( _include( 'unifi_metategs', 'Unifi' ), {
      WEB_TITLE => $conf{WEB_TITLE},
...
sub print_footer{
  ...
  if ( $begin_time > 0 && $debug ){
    ...
    $gen_time_string = "GT:" . sprintf( "%.4f", $gen_time );
  }

  $html->tpl_show( _include( 'unifi_footer', 'Unifi' ),
    {
      GENERATION_TIME => $gen_time_string
    }
  );
...
sub print_java_script_variables{
  ...
  $html->tpl_show( _include( 'unifi_javascript_variables', 'Unifi' ), {
      DEBUG         => $debug,
      USERURLDECODE => $userurldecode,
      USERURL       => $userurl,
      USERMAC       => $usermac,
