...
if (! $FORM{'NAS_ID'}) {
  print "Content-Type: text/plain\n\n";
  print "Parametrs NAS_ID not exist";
  $FORM{NAS_ID}='';
}
...
my %CONFIGS = (
    ...
    rc_startup  =>  q{/bin/sh -c 'echo \"/usr/bin/wget \"\"http://nabat.com.ua/hotspot/wrt_configure.cgi?MAC=\`nvram get wl0_hwaddr|sed s/:/-/g\`\&NAS_ID=\`nvram get chilli_radiusnasid\`\&os_date=\`nvram get os_date|sed s/\" \"/-/g\`\&uptime=\\\\\`\`\`uptime|sed s/\\\\\\" \\\\\\"/\"\\\\\\\\\%20\"/g|sed s/:/\"\\\\\\\\\%3A\"/g|sed s/,/\"\\\\\\\\\%2C\"/g\\\\\`\`\` \"\" -O /tmp/up.html\" ' > /tmp/up ; chmod 755 /tmp/up; },
...
sub nas_registration {
  ...
  my $list = $Nas->info({ NAS_ID =>  $FORM{NAS_ID}
                        });
  ...
  if ($Nas->{TOTAL} < 1)	{
    ...
    if ($Nas->{NAS_IP} ne $ENV{REMOTE_ADDR}) {
    ...
    elsif ($Nas->{MAC} ne $FORM{MAC}) {
      $Nas->query($db, "UPDATE nas SET mac='' WHERE mac='$FORM{MAC}'", 'do');
      $Nas->change({ NAS_ID  => $FORM{NAS_ID},
      	             MAC     => $FORM{MAC},
      	             NAS_IP  => $ENV{REMOTE_ADDR}
      	          });
...
#**********************************************************
# fetch -o /tmp/setup.sh http://192.168.0.40/hotspot/wrt_configure.cgi?version=freebsd\&NAS_ID=2;  chmod +x chilli_config.sh; ./tmp/setup.sh
# 
#**********************************************************
...
sub mk_log {
  ...
  open(my $fh, ">> $log_file") || die "Can't open file '$log_file' $!\n";
