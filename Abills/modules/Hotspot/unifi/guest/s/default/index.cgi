...
BEGIN {
  ...
  unshift(@INC, $libpath, $libpath . "Abills/$sql_type/", $libpath . 'Abills/modules/', $libpath . "lib/");
...
my $db = Abills::SQL->connect($conf{dbtype}, $conf{dbhost}, $conf{dbname}, $conf{dbuser}, $conf{dbpasswd}, { %conf, CHARSET => ($conf{dbcharset}) ? $conf{dbcharset} : undef });
