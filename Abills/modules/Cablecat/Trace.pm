...
=head2 SYNOPSIS
  ...
  This package aggregates function to trace pathes for cables
...
sub resolve_commutation_path {
  ...
  if (!$start_link){
    _bp('', "Not found " . join('_', @next_link_array));
  }
...
sub get_path_beetween {
  ...
  if ( !$first_commutation_list || !$second_commutation_list ) {
    ...
    $self->{errstr} = 'Element ' . (uc $type || q{}) . " " . ($id || '0')
                        . ' is not present on any commutation';
    return 0;
  };
...
sub _get_another_element_for_link {
  ...
  my $is_first = (
    ($link->{element_1_type} eq $this_el_type && $link->{element_1_id} eq $this_el_id)
      ||
