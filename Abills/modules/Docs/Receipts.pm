...
sub docs_receipt_form {
  ...
  my $num = 0;
  my $total_sum = ($users->{DEPOSIT} !~ /^[0-9\.\,]+$/xm) ? 0 : (($users->{DEPOSIT} && $users->{DEPOSIT} < 0) ? abs($users->{DEPOSIT}) : 0 - $users->{DEPOSIT});
  my $amount_for_pay = 0;
  my $TO_D;
  foreach my $line (@$list) {
    ...
    $date =~ s/\s+\d+:\d+:\d+//xg;
    $table->addrow(
      $html->form_input("ORDER_" . $line->{fees_id} || q{}, ($line->{dsc} ? $line->$line->{dsc} : q{}), { TYPE => 'hidden', OUTPUT2RETURN => 1 })
        . $html->form_input("SUM_" . $line->{fees_id} || q{}, ($line->{sum} ? $line->{sum} : q{}), { TYPE => 'hidden', OUTPUT2RETURN => 1 })
        . $html->form_input("FEES_ID_" . $line->{fees_id} || q{}, ($line->{fees_id} ? $line->{fees_id} : q{}), { TYPE => 'hidden', OUTPUT2RETURN => 1 })
        . $html->form_input("IDS", ($line->{fees_id} ? $line->{fees_id} : q{}), { TYPE => 'checkbox', STATE => 1, OUTPUT2RETURN => 1 }) . $num,
      ($line->{login} ? $line->{login} : q{}), ($line->{date} ? $line->{date} : q{}), ($line->{dsc} ? "$line->{dsc} $date" : "$date"), ($line->{sum} ? $line->{sum} : q{})
    );
  ...
  if ($FORM{NEXT_PERIOD}) {
    ...
    if ($users->{ACTIVATE} ne '0000-00-00') {
      ($Y, $M, $D) = split(/-/x, POSIX::strftime("%Y-%m-%d", localtime((POSIX::mktime(0, 0, 0, $D, ($M - 1), ($Y - 1900), 0, 0,
  ...
  my $deposit = ($users->{DEPOSIT} && $users->{DEPOSIT} =~ /^[0-9\.\,]+$/xm) ? $users->{DEPOSIT} : 0;
...
sub docs_receipt_print {
  ...
  if ($Docs->{ORDERS}) {
    ...
    foreach my $line (@ORDERS) {
      ...
      my $sum = sprintf("%.2f", $line->[3] * $line->[4]);
      $Docs->{ORDER} .= <<"TEXT";
      <tr><th align='right'>$i</th><td align='left'>$line->[1]</td><td align='center'>$units[$line->[2]]</td>
          <td align='right'>$line->[3]</td><td align='right'>$line->[4]</td><td align='right'>$sum</td></tr>
...
sub docs_receipt_print_list {
  ...
  foreach my $line (@{$receipt_list}) {
    ...
    my ($y, $m, $d) = split(/\-/x, $line->{datetime}, 3);
  ...
  if ($receipt_content =~ /No\s+such\s+t/xm) {
