...
sub docs_tax_invoice {
  ...
  if ($FORM{create}) {
    ...
    elsif (!$FORM{SUM_1} && ($FORM{SUM} =~ /\d+/xm && $FORM{SUM} < 0.01)) {
...
sub docs_tax_exports {
  ...
  if ($FORM{d}) {
    my ($y, $m, undef) = split(/-/x, ($FORM{MONTH}) ? $FORM{MONTH} . '-01' : $DATE);
    my $file_prefix = "26590025388695J120150510000000011";
    my $filename = $file_prefix . "$m$y" . '2659.xml';

    my ($fill_y, $fill_m, $fill_d) = split(/-/x, ($FORM{DATE}) ? $FORM{DATE} : $DATE);
