...
BEGIN {
  #use lib ; # Assuming we are in /usr/abills/Abills/modules/Wordpress/
  unshift ( @INC, "../../../lib/" );
}
...
sub get_themes {
  my ($self) = @_;
  return $xmlrpc->call('abills.get_custom_themes',  @{$self->{authenticate}});
}
...
sub get_current_theme {
  my ($self) = @_;
  return $xmlrpc->call('abills.get_current_theme',  @{$self->{authenticate}});
}
...
sub get_custom_options {
  ...
  return $xmlrpc->call('abills.get_custom_options', @{$self->{authenticate}});
}
...
sub update_custom_options {
  ...
  return $xmlrpc->call('abills.update_custom_options', @{$self->{authenticate}}, $new_options);
}
