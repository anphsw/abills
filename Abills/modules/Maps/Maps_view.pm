...
sub new {
  ...
  $Maps_info = Maps::Maps_info->new($db, $admin, $CONF, $attr);
...
sub show_map {
  ...
  my $map_show = $html->tpl_show(::_include('maps_main', 'Maps'), {
...
#**********************************************************
=head2 maps_get_objects_to_show($attr)
...
sub maps_get_objects_to_show {
  ...
  if ($attr->{DATA} && $attr->{DONE_DATA}) {
    ...
    if (ref $attr->{DATA} eq 'ARRAY') {
      $form->{OBJECT_TO_SHOW} = $self->maps_get_custom_array_objects($attr);
    }
    elsif (ref $attr->{DATA} eq 'HASH') {
      $form->{OBJECT_TO_SHOW} = $self->maps_get_custom_hash_objects($attr);
    }
  }
...
sub maps_get_build_objects {
  ...
  $result = $Maps_info->maps_builds_show({
    %{$attr},
    EXPORT      => 1,
    RETURN_HASH => 1,
  ...
  $result = $Maps_info->maps_builds2_show({
    %{$attr},
    EXPORT      => 1,
    RETURN_HASH => 1,
    TO_SCREEN   => defined $attr->{TO_SCREEN} ? $attr->{TO_SCREEN} : 1
...
#**********************************************************
=head2 maps_get_custom_hash_objects($attr)
...
=cut
#**********************************************************
sub maps_get_custom_hash_objects {
  my $self = shift;
  my ($attr) = @_;
  ...
  foreach my $key (keys %{$attr->{DATA}}) {
    $build_objects = $self->maps_get_build_objects({
    ...
    if ($attr->{MAP_SHOW_ITEMS} && $build_objects && ref $build_objects eq 'ARRAY') {
      foreach my $build (@{$build_objects}) {
        my $info = _maps_get_custom_info($attr->{DATA}{$key}, {
...
#**********************************************************
=head2 maps_get_custom_array_objects($attr)
...
=cut
#**********************************************************
sub maps_get_custom_array_objects {
  my $self = shift;
  my ($attr) = @_;
  ...
  foreach my $object (@{$attr->{DATA}}) {
    ...
    $result = $self->maps_get_build_objects($object);
...
#**********************************************************
=head2 _maps_get_custom_info($attr)
...
sub _maps_get_custom_info {
  ...
  my $info = $Auxiliary->maps_point_info_table({
    OBJECTS           => $objects,
