...
my $Msgs = Msgs::db::New->new($db, $admin, \%conf);
...
sub msgs_redirect_filter {
  ...
  if ($conf{MSGS_REDIRECT_FILTER_DEL} && $attr->{DEL}) {
    ...
    if ($conf{MSGS_REDIRECT_FILTER_DEL} =~ /EXEC:(.+)/xm) {
      $cmd = $1;
    }
  }
  #Add filter
    ...
    $action = 'add';
    my $rad = q{};
    ($cmd, $rad) = $conf{MSGS_REDIRECT_FILTER_ADD} =~ /(EXEC:[a-zA-Z0-9\_\-\.\/\s+\>\%\"\'\@\=]+)?;?(RAD:.+)?/xm;
...
#**********************************************************
=head2 msgs_user_new($attr) - Count New messages for user
...
sub msgs_user_new {
  ...
  if ($conf{MSGS_UNREAD_FORWARD}) {
    my @RULES_ARR = split(/;/x, $conf{MSGS_UNREAD_FORWARD});
    foreach my $rule (@RULES_ARR) {
      next if (!$rule);
      my ($USER_GID, $MESSAGE_CHAPTER, $URL) = split(/:/x, $rule, 3);
...
sub msgs_admin_new {
  ...
  my $msgs_permissions = $Msgs->permissions_list($aid); # if(!%msgs_permissions);

  my $msgs_list = $Msgs->messages_new({
    ADMIN_READ    => '0000-00-00  00:00:00',
    GID           => $admin->{GID},
    CHAPTER       => (!$msgs_permissions->{4}) ? '_SHOW' : join(',', keys %{$msgs_permissions->{4}}),
    SHOW_CHAPTERS => 1,
  ...
  foreach my $line (@{$msgs_list}) {
    ...
    if ($chapter_name && length($chapter_name) > 25) {
      $chapter_name = join('', @{[ split(//x, $chapter_name) ]}[0 ... 24]);
  ...
  if (!$FORM{xml} && !$FORM{json}) {
    ...
    #   "closedAdmin&watchers&planTime&total&inWork&open&unmaked&closed&externalChatId=0";

    my $url = '/api.cgi/msgs/list?snakeCase=1&pageRows=20&clientId'
      #. '&chapterName'
      . '&datetime'
      . '&state_id=0'
      #. '&resposibleAdminLogin'
      . '&priorityId'
      #. '&domainId'
      #. '&message'
      . '&adminRead'
