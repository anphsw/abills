package Msgs::Plugins::Msgs_maps;
...
sub new {
  ...
  use Maps::Maps_view;
  $Maps_view = Maps::Maps_view->new($db, $admin, $CONF, { HTML => $html, LANG => $lang });
...
sub plugin_show {
  ...
  return if !$Msgs->{ID};
  return '' if (!in_array('Maps', \@::MODULES));
  return '' if ($admin->{MODULES} && !$admin->{MODULES}{Maps});
  ...
  if (!$Msgs->{LOCATION_ID}) {
    ...
    $Msgs->{LOCATION_ID} = $message->[0]{LOCATION_ID} || $message->[0]{LOCATION_ID_MSG} || 0;
  }
