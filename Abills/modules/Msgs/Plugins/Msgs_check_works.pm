package Msgs::Plugins::Msgs_check_works;
...
sub new {
  ...
  my $self = { PLUGIN_NAME => 'Msgs_check_works' };
...
sub employee_check_ticket_works {
  ...
  return '' if (!in_array('Employees', \@::MODULES));
  return '' if ($admin->{MODULES} && !$admin->{MODULES}{Employees});
