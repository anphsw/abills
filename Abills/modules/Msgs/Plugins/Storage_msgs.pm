package Msgs::Plugins::Storage_msgs;
...
sub new {
  ...
  return $self if (!in_array('Storage', \@::MODULES));
  return $self if ($admin->{MODULES} && !$admin->{MODULES}{Storage});
...
sub plugin_show {
  ...
  return '' if (!in_array('Storage', \@::MODULES));
  return '' if ($admin->{MODULES} && !$admin->{MODULES}{Storage});
