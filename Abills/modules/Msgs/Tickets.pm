...
sub msgs_admin {
  ...
  if ( $FORM{CHANGE_MSGS_TAGS} ) {
    $Msgs->quick_replys_tags_add({ IDS => $FORM{TAGS_IDS}, MSG_ID => $FORM{chg} });
    $html->message('info', $lang{INFO}, "$lang{ADD} $Msgs->{TOTAL} $lang{TAGS}") if ( !$Msgs->{errno} );
    $html->message('err', $lang{ERROR}, "$lang{NO} $lang{ADD} $lang{TAGS}") if ( $Msgs->{errno} );
  }

  if ( $FORM{MSG_PRINT_ID} ) {

    msgs_ticket_form({ MSG_PRINT_ID => $FORM{MSG_PRINT_ID}, UID => $FORM{UID} });
  ...
  if ( $FORM{deligate} ) {
    ...
    if ( $FORM{WATCH} ) {
      ...
      # change_resposible will need aid of current responsible admin,
...
sub msgs_admin_add {
  ...
  if ( $FORM{add} || $FORM{PREVIEW} ) {
    ...
    #Send message
      ...
      foreach my $line ( @{$list} ) {
        ...
        my $user_pi = $users->pi({ UID => $line->{uid}, COLS_NAME => 1, COLS_UPPER => 1 });
        my $dv_info = {};
        if ( in_array('Dv', \@MODULES) ) {
          require Dv;
          my $Dv = Dv->new($db, $admin, \%conf);
          $dv_info = $Dv->info($line->{uid}, { COLS_NAME => 1, COLS_UPPER => 1 });
        }
  ...
  if ( !$FORM{UID} || $FORM{UID} =~ /;/ ) {
    ...
    $Msgs->{ADDRESS_FORM} = $html->tpl_show(
      ...
      {
        CONTENT =>
        $html->tpl_show(templates('form_address_sel'), undef, { OUTPUT2RETURN => 1, ID => 'form_address_sel' }),
        NAME    => $lang{ADDRESS},
        ID      => 'ADDRESS_FORM',
    ...
    $Msgs->{SEND_EXTRA_FORM} = $html->tpl_show(_include('msgs_send_extra', 'Msgs'), $Msgs,
  ...
  # This code will show just a list of EXISTING plugins
  my $sender_send_types = $Sender->notification_type();
  %send_types = (%send_types, %{$sender_send_types});
  
  # TODO: This code can show only plugins that WILL send message
#  my @sender_send_types = $Sender->available_types();
#  %send_types = (%send_types, (map { $Abills::Sender::Core::TYPE_ID_FOR_PLUGIN_NAME{$_} => $_ } @sender_send_types));
...
sub msgs_ticket_show {
  ...
  # TODO: remove when result folmer list will be fixed (#899)
  # Fix missing $FORM{UID}
  ...
  $Msgs->{INNER_MSG_TEXT} = ($Msgs->{INNER_MSG})                                                       ? $html->element(
      'span', '',
  ...
  $Msgs->{MSG_PRINT_BTN} = $html->button(
    '',
    "qindex=$index&UID=$Msgs->{UID}&MSG_PRINT_ID=$message_id&header=2" . (($conf{DOCS_PDF_PRINT}) ? '&pdf=1' : ''),
  ...
  if ($permissions{4} || $conf{MSGS_TAGS_NON_PRIVILEGED}) {
    $Msgs->{ADD_TAGS_BTN} = $html->button('',
      'qindex=' . get_function_index('msgs_quick_replys_tags') . "&header=2&MSGS_ID=$message_id&UID=$Msgs->{UID}",
      {
        LOAD_TO_MODAL => 1,
        class         => 'btn btn-default',
  ...
  $Msgs->{EXT_INFO} = $html->tpl_show(_include('msgs_managment', 'Msgs'), { %{$users}, %{$Msgs} },
    { OUTPUT2RETURN => 1 });
  ...
  foreach my $line ( @{$list} ) {
    ...
    my $new_topic_button = '';
    if ( $permissions{7} && $permissions{7}->{1} && $Msgs->{UID} ) {
      $new_topic_button = $html->button($lang{CREATE_NEW_TOPIC},
        "&index=$index&chg=$message_id&UID=$Msgs->{UID}&make_new=$line->{id}&chapter=$Msgs->{CHAPTER}",
    ...
    push @REPLIES, $html->tpl_show(
        _include('msgs_reply_show', 'Msgs'),
        {
          ...
          DATE       => $line->{datetime},
          CAPTION    =>
          convert($line->{caption}, { text2html => 1, json => $FORM{json} }) . "  #  $Msgs->{ID}  $line->{id}",
          PERSON     =>
          ($line->{creator_id} || q{}) . ' ' . (($line->{aid}) ? " ($lang{ADMIN})" . (($line->{inner_msg}) ? "  $lang{PRIVATE}" : '') : ""),
          MESSAGE    => msgs_text_quoting($line->{text},
            1) . (($line->{filename} && $line->{content_type} && $line->{content_type} =~ /ima/) ? $html->img("$SELF_URL?qindex=$index&ATTACHMENT=$line->{attachment_id}") : ''),
          ...
          DELETE     =>
          $html->button($lang{DEL}, "&index=$index&chg=$message_id&reply_del=$line->{id}&UID=$Msgs->{UID}",
            { MESSAGE => "$lang{DEL}  $line->{id}?", BUTTON => 1 }),
          ATTACHMENT => ($line->{filename})                                                                                             ? "$lang{ATTACHMENT}: " . $html->button(
              $line->{filename}, "qindex=$index&ATTACHMENT=$line->{attachment_id}", { TARGET =>
                '_new' }) . " ($lang{SIZE}:  " . int2byte($line->{content_size}) . ')' . (($line->{coordx} + $line->{coordy} > 0) ? ' '
                . $html->button($lang{MAPS}, "index="
                #              . get_function_index( 'maps_show_map' ) . "&COORDX=$line->{coordx}&COORDY=$line->{coordy}"
                ,
                { TARGET =>
                  '_NEW' })                                                                                                       : '') : '',
          COLOR      => $msg_color,
          RUN_TIME   =>
            ($line->{run_time} && $line->{run_time} ne '00:00:00') ? "$lang{RUN_TIME}: $line->{run_time}" : '',
          ADMIN_MSG  => $line->{aid}
        },
      ...
      _include('msgs_reply', 'Msgs'),
      {
        ...
        RUN_TIME_FORM   => $html->tpl_show(
          templates('form_row'),
          {
            ...
            VALUE => $html->form_input('RUN_TIME', "$run_time",
  ...
  if ( $Msgs->{TOTAL} ) {
    foreach my $msg_tag ( @{$msg_tags_list} ) {
      $Msgs->{MSG_TAGS} .= ' ' . $html->element('span', $msg_tag->{reply},
        { 'class' => "label", 'style' => "background-color:$msg_tag->{color};border-color:white;font-weight: bold" });
    }
  }
  else {
    $Msgs->{MSG_TAGS_DISPLAY_STATUS} = 'none';
  }
...
sub _msgs_change_responsible {
  ...
  my $previous_responsible_aid = $message_info->{resposible};

  # Check if resposible has been changed to another admin
  return 1 if (!$new_responsible_aid || $previous_responsible_aid eq $new_responsible_aid);
