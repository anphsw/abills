...
#  $REQUEST_HASH{request}{auth}{time}  = $date;                                    # now time
  ...
  $request{request}{body}{user_id} = $user->{$self->{conf}{PAYSYS_IPAY_ACCOUNT_KEY} || 'UID'};
  ...
  if ($action eq 'Check' || $action eq 'List') {
    ...
    $request{request}{body}{pmt_info}{acc}     = $user->{$self->{conf}{PAYSYS_IPAY_ACCOUNT_KEY} || 'UID'};
...
sub user_portal_special {
  ...
  if ($user_->{PHONE}) {
    ...
    if ($user_->{PHONE} !~ /^380/ || length($user_->{PHONE}) != 12) {
      return $html->message('err', $self->{lang}->{ERR_WRONG_PHONE},
        "$self->{lang}->{PHONE}: 380XXXXXXXXX", { OUTPUT2RETURN => 1, ID => 1788  });
    }
  }
  else {
    return $html->message('err', $self->{lang}->{ERR_WRONG_PHONE},
      "$self->{lang}->{PHONE}: 380XXXXXXXXX", { ID => 1789, OUTPUT2RETURN => 1 });
  }
  ...
  # make payment if registered
    ...
    if($attr->{ipay_purchase} == 1){
      $html->message('info', $self->{lang}->{SUCCESS}, "$self->{lang}->{SUCCESS} $self->{lang}->{TRANSACTION}: $attr->{pmt_id}", { ID => 3333  });
    }
  ...
  if ($user_status eq 'exists') {
    my $cards_list   = '';
    my $card_checked = 0;
    my $json_list_string = $self->create_request_params('List');
    ...
    my $json_add_card_by_url = $self->create_request_params('AddcardByURL');
  ...
  # INVITE PROCESSING
    ...
    my $json_invite_by_url_string = $self->create_request_params('InviteByURL');
...
<info></info>
    ...
    my $account_key = $self->{conf}{PAYSYS_IPAYC_ACCOUNT_KEY} || 'UID';
    my %DATA = (
      $account_key   => $account,
