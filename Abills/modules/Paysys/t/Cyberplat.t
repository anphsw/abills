...
#Web test check
$result = Abills::Fetcher::web_request("https://$url/paysys_check.cgi?action=check&receipt=$random_number&number=$user_id&amount=1.00", {
  INSECURE => 1,
});
...
$result = Abills::Fetcher::web_request("https://$url/paysys_check.cgi?action=payment&receipt=$txn_num&number=$user_id&amount=1.00&date=2005-09-20T15:53:00", {
  INSECURE => 1,
});
...
my $response = Abills::Fetcher::web_request("https://$url/paysys_check.cgi?action=payment&receipt=$receipt&number=$user_id&amount=1.00&date=2005-09-20T15:53:00", {
  INSECURE => 1,
});
my $req_xml_pyament_id = XML::Simple::XMLin($response, ForceArray => 0, KeyAttr => 1);
#Web cancel check
$result = Abills::Fetcher::web_request("https://$url/paysys_check.cgi?action=cancel&receipt=$receipt", {
  INSECURE => 1,
});
