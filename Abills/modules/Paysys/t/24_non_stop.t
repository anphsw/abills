...
if (!$conf{PAYSYS_V4}) {
  ...
  $Payment_plugin = Paysys::systems::24_non_stop->new($db, $admin, \%conf);
}
else {
  require Paysys::Plugins::24_non_stop;
  $Payment_plugin = Paysys::Plugins::24_non_stop->new($db, $admin, \%conf);
}
if ($debug > 3) {
