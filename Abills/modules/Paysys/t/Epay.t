...
my $Epay = Paysys::systems::Epay->new($db, $admin, \%conf, {
  CUSTOM_NAME => $attr->{NAME} || '',
  CUSTOM_ID   => $attr->{ID} || '',
});

# checking function check with valid account
my $result = $Epay->proccess(
  {
    account => "$user_id",
    command => 'check',
    sum     => '1.00',
