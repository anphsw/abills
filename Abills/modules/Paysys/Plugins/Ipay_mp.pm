package Paysys::Plugins::Ipay_mp;
#*********************** ABillS ***********************************
...
sub new {
  ...
  # legacy schema for iPay masterpass maybe will be reworked in future
  if ($attr->{HTML}) {
    $html = $attr->{HTML};
  }
  ...
  if ($attr->{ERRORS}) {
    ...
    $Errors = Control::Errors->new($db, $admin, $CONF, { module => 'Paysys' });
  }
...
sub proccess {
  ...
  if ($debug > 0) {
    $Paysys_Core->mk_log($xml, { PAYSYS_ID => $PAYSYSTEM_ID, REQUEST => 'Request' });
  }
  ...
  my $result = q{};
  if($request->{check}){
    my ($check_status, $user_info) = $Paysys_Core->paysys_check_user({
      PAYSYS_ID   => $PAYSYSTEM_ID,
...
<info></info>
    ...
    my $account = '';
    #TODO: conf_gid_split based on settlements[0].statement.smch_id
    ...
    elsif ($payment_status == 6 || $payment_status == 4) {
      $error_code = 6;
    }
    elsif ($payment_status == 9) {
      $Paysys_Core->paysys_pay_cancel({
        TRANSACTION_ID => "$PAYSYSTEM_SHORT_NAME:$payment_id"
      });
...
sub has_test {
  ...
  if ($attr->{_POST_}) {
    my ($salt, $signature) = $self->mk_sign({ test_key => 1 });

    $attr->{_POST_} =~ s/<salt>(.+)<\/salt>/<salt>$salt<\/salt>/g;
    $attr->{_POST_} =~ s/<sign>(.+)<\/sign>/<sign>$signature<\/sign>/g;
  }
  ...
  my %params_hash = (
    ...
    '2_PAY'      => {
      _POST_      => {
        name    => '_POST_',
        val     => $requests[0]->{request},
        tooltip => $requests[0]->{name}
      },
      result      => [],
      result_type => 'xml',
    },
    '3_CANCEL'   => {
      _POST_      => {
        name    => '_POST_',
        val     => $requests[2]->{request},
        tooltip => $requests[2]->{name}
      },
      result      => [],
      result_type => 'xml',
    },
...
sub fast_pay_link {
  ...
  if (!$self->{conf}->{PAYSYS_IPAY_MERCHANT_ID}) {
    return $Errors->throw_error(1170002);
  }
  ...
  $attr->{USER_ID} = $acc;
  $link .= "bill_id=$self->{conf}->{PAYSYS_IPAY_MERCHANT_ID}&acc=$acc&template_invoice=$attr->{SUM}&invoice=$attr->{SUM}";
  return {
    URL            => $link,
