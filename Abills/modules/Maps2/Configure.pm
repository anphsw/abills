...
use Maps2::Shared qw/:all MAPS_ICONS_DIR MAPS_ICONS_DIR_WEB_PATH LAYER_ID_BY_NAME CLOSE_OUTER_MODAL_SCRIPT/;
...
=cut
#**********************************************************
sub maps2_auto_coords {

  $html->tpl_show(_include('maps2_coords_form', 'Maps2'), {
    DISTRICTS_SELECT => _maps2_address_districts(\%FORM),
    STREETS_SELECT   => _maps2_address_streets(\%FORM)
  });
...
#**********************************************************
=head2 _maps2_address_districts()

=cut
#**********************************************************
sub _maps2_address_districts {
  my ($attr) = @_;
...
#**********************************************************
=head2 _maps2_address_streets()

=cut
#**********************************************************
sub _maps2_address_streets {
  my ($attr) = @_;
...
#**********************************************************
=head2 _maps2_icon_filename_select()
...
#**********************************************************
=head2 maps2_point_types_main()

=cut
#**********************************************************
sub maps2_point_types_main {
  ...
  if ($FORM{change}) {
    ...
    $html->tpl_show(_include('maps2_point_types', 'Maps2'), {
      NAME        => _translate($type->[0]{name}),
      COMMENTS    => $type->[0]{comments},
      ICON_SELECT => _maps2_icon_filename_select({ NAME => 'ICON', NO_EXTENSION => 1, ICON => $type->[0]{icon} }),
  ...
  foreach (@{$types_list}) {
    ...
    $types_table->addrow($_->{id}, _translate($_->{name}),
      $_->{icon} ? _maps2_get_icon_img($_->{icon}) : '', $_->{comments}, $chg_button);
  }
...
#**********************************************************
=head2 _maps2_get_icon_img()

=cut
#**********************************************************
sub _maps2_get_icon_img {
  my ($icon_name, $attr) = @_;
