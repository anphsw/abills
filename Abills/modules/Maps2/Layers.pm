...
use Maps2::Auxiliary qw/maps2_point_color maps2_point_info_table maps2_points_count maps2_load_module/;
...
sub maps2_districts_main {
  ...
  if ($FORM{add}) {
    my $new_point_id = $Auxiliary->maps2_add_external_object(LAYER_ID_BY_NAME->{DISTRICT}, \%FORM);
...
=cut
#**********************************************************
sub maps2_get_objects {
  my ($attr) = @_;

  return if (!$FORM{MODULE} || !$FORM{FUNCTION});
  
  my $module = maps2_load_module($FORM{MODULE});
