package Maps2::Auxiliary;

=head1 NAME

  Maps2::Auxiliary - Auxiliary functions for Maps2
...
sub maps2_point_info_table {
  ...
  foreach my $u (@{$objects}) {
    ...
    for (my $i = 0; $i <= $#{$table_titles}; $i++) {
      ...
      $value = _maps2_get_value_for_table($html, {
        OBJECT        => $u,
...
sub maps2_load_info {
  ...
  my $module = maps2_load_module($module_name);
