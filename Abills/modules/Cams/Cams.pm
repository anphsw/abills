...
sub add_cam {
  ...
  cmd("/usr/abills/libexec/cams_management.pl ADD=$name STREAM=$stream_url $logo1_option $logo2_option $rotate_params");
...
sub delete_cam {
  ...
  if ($attr->{HASH_NAME}) {
    cmd("/usr/abills/libexec/cams_management.pl DELETE=$attr->{HASH_NAME}");
  }
  else {
    return 0;
  }
