...
$db = DBI->connect( "dbi:mysql:dbname=$conf{dbname}", "$conf{dbuser}", "$conf{dbpasswd}" )
...
foreach my $table (@tables) {
  ...
  while (my ($src_addr, $dst_addr, $src_port, $dst_port, $protocol, $size,
    $f_time, $s_time_, $nas_id) = $q->fetchrow_array()) {

    if ($FORMAT eq 'tab_delimeter') {
      print "$src_addr\t$src_port\t$dst_addr\t$dst_port\t$protocol\t$size\t$f_time\t$s_time_\t$nas_id";
    }
    else {
      printf( "%-15s|%-5s|%-15s|%-5s|%4s|%-8s|%-19s|%-19s|%-3s|\n", $src_addr, $src_port, $dst_addr,
        $dst_port, $protocol, $size, $f_time, $s_time_, $nas_id );
    }
    $total += $size;
  }
