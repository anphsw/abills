...
use Abills::Base qw(check_time parse_arguments gen_time next_month
...
if (! $ENV{HTTP_KEY}) {
  print "ERROR: No API key AID: $conf{SYSTEM_ADMIN_ID}\n";
}
...
if (defined($argv->{POSTPAID_INVOICES})) {
  ...
  prepaid_invoices($argv) if (!$argv->{COMPANY_ID});
...
sub prepaid_invoices_company {
  ...
  foreach my $line (@{$list}) {
    ...
    foreach my $internet_info (@{$internet_list}) {
      ...
      if ($conf{DOCS_PERIOD_UNPAYMENT_INVOICE}) {
        my $unpayment_count = _check_unpayment_invoices($uid);
        return if ($unpayment_count && $unpayment_count == $conf{DOCS_PERIOD_UNPAYMENT_INVOICE});
      };
...
sub postpaid_invoices {
  ...
  $LIST_PARAMS{DEPOSIT} = $attr->{DEPOSIT} if ($attr->{DEPOSIT});
  $LIST_PARAMS{PHONE} = $attr->{PHONE} if (defined($attr->{PHONE}));
  ...
  if ($attr->{MODULE}) {
    my Internet $mod_name = $attr->{MODULE};
    require $mod_name . '.pm';
    $mod_name->import();
    my Internet $Module_name = $mod_name->new($db, $admin, \%conf);

    $USER_LIST_PARAMS{TP_ID} = $attr->{TP_ID} if ($attr->{TP_ID});
  ...
  foreach my $line (@{$user_list}) {
    ...
    if ($attr->{ADDRESS2}) {
      ...
      $full_address .= $build_delimiter . ($line->{address_flat} || '');
    }

    my $month_fee = ($FEES_LIST_HASH{ $line->{uid} }) ? $FEES_LIST_HASH{ $line->{uid} } : '0.00';
    my $deposit = $line->{deposit} || 0;

    my $debt = ($deposit < 0) ? sprintf("%.2f", $deposit) : 0.00;
    my $amount_for_pay = 0;
    ...
    if ($deposit) {
      $amount_for_pay = abs(($deposit > $total_sum) ? 0 : $deposit - $total_sum);
    }
    else {
      $amount_for_pay = $month_fee;
    }
    ...
    if ($deposit < 0) {
      push @MULTI_ARR, {
        ...
        MONTH_FEE           => $month_fee,
        TOTAL_SUM           => sprintf("%.2f", abs($total_sum)),
        AMOUNT_FOR_PAY      => sprintf("%.2f", $amount_for_pay),
        DEBT                => $debt,
        CONTRACT_ID         => $line->{contract_id},
        CONTRACT_DATE       => $line->{contract_date},
        DATE                => $DATE,
        DATE_EURO_STANDART  => "$d.$m.$Y",
        #FULL_ADDRESS        => $full_address,
        ADDRESS_FULL        => $line->{ADDRESS_FULL} || $full_address,
        YEAR                => $Y,
        ...
        MONTH_LIT           => $MONTHES_LIT[ int($m) - 1 ],
        MONTH_LIT_NOM       => $MONTHES_LIT_NOM[ int($m) - 1 ]
      };
      $doc_num++;
...
sub multi_tpls {
  ...
  $MULTI_ARR = multi_docs_former($MULTI_ARR, $argv->{PAGE_DOCS});

  if ($debug > 4) {
    print @$MULTI_ARR;
    foreach my $items (@$MULTI_ARR) {
      print "\n----------------------------------\n";
...
#**********************************************************
=head2 multi_docs_former($records, $page_docs)
...
=cut
#**********************************************************
sub multi_docs_former {
  my ($records, $page_docs)=@_;
  ...
  foreach my $items (@{ $records }) {
    while(my ($k, $v) = each %$items) {
      $formed_page{ uc($k).'_' . $i }=$v;
    }

    if ($i && !($i % $page_docs)) {
      push @formed_doc, { %formed_page };
      %formed_page = ();
      $i           = 1;
    }
    else {
...
#**********************************************************
=head2 _check_unpayment_invoices()
...
=cut
#**********************************************************
sub _check_unpayment_invoices {
  my ($uid) = @_;

  my $invoices_list = $Docs->invoices_list({
    UID       => $uid,
    PAGE_ROWS => $conf{DOCS_PERIOD_UNPAYMENT_INVOICE},
    COLS_NAME => 1
  });
  return if (!$Docs);

  my $unpayment_count = 0;
  foreach my $invoice (@$invoices_list) {
    if (!defined $invoice->{'payment_sum'}) {
      $unpayment_count++;
    }
  }

  if (in_array('Internet', \@MODULES) && $unpayment_count == $conf{DOCS_PERIOD_UNPAYMENT_INVOICE}) {
    require Internet;
    Internet->import();
    my $Internet = Internet->new($db, $admin, \%conf);
    ...
    print "UID: $uid has $unpayment_count unpayment invoices. Service status is changed to disable\n" if ($debug > 2);
  }

  return $unpayment_count;
}


#**********************************************************
=head2 prepaid_invoices2($attr) - Make invoice for users
...
sub prepaid_invoices {
  ...
  foreach my $u (@{$users_list}) {
    my $uid = $u->{UID};
    print "UID: $uid LOGIN: $u->{LOGIN} FIO: $u->{FIO}\n" if ($debug > 2);
    ...
    if (! $attr->{INVOICE2ALL} && !$Docs->{PERIODIC_CREATE_DOCS}) {
      print "Skip create docs INVOICE2ALL: " . ($attr->{INVOICE2ALL} || q{})
    ...
    my %invoice_data = (
      UID        => $uid,
      create     => 1,
      SEND_EMAIL => $attr->{INVOICE2ALL} || $Docs->{SEND_DOCS},
      pdf        => 1,
      CUSTOMER   => '-',
      EMAIL      => $Docs->{EMAIL}
    );

    my $services = ::get_services($u, { IPTV_SHOW_FREE_TPS => 1 });
    my $num = 1;
    my @ids = ();
    ...
    foreach my $service (sort @{$services->{list}}) {
      if ($service->{ID}) {
        ...
        my $order = $service->{SERVICE_NAME};

        if ($debug > 3) {
          print "$name} = $service->{ID} / $order \n";
        }

        my $sum = $service->{SUM} || 0;
        $invoice_data{'ORDER_' . $num} = $order;
        $invoice_data{'SUM_' . $num} = $sum;
        push @ids, $num;
        $num++;
      }
    }
    ...
    #Add debetor invoice
    if ($u->{DEPOSIT} && $u->{DEPOSIT} < 0) {
      print "  DEPOSIT: $u->{DEPOSIT} SEND: $invoice_data{SEND_EMAIL}\n" if ($debug > 2);
      $invoice_data{'ORDER_' . $num} = $lang{DEBT};
      $invoice_data{'SUM_' . $num} = abs($u->{DEPOSIT});
    }
