...
mikrotik_skip () {
  ...
  CMD_CONNECT="${SSH} -p ${PORT} -o ConnectTimeout=10 -i /usr/abills/Certs/id_rsa.${USER_NAME} ${USER_NAME}@${HOST} ";
...
#
#if [ "${ACTION}" = "add" ]; then
#  /usr/bin/ssh -i /usr/abills/Certs/id_dsa.abills_admin -o StrictHostKeyChecking=no -q abills_admin@${host}  "/usr/local/bin/sudo /usr/abills/misc/msgs_filter.sh ${ACTION} ${USER_UID}"
#else
