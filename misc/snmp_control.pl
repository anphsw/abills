...
sub dlink_port_speed {
  ...
  if ($FILTER_ID !~ /PORT_SPEED:(\S+)/) {
    ...
    #dlink 3526 (скорость кратна 1ћбит/с)
  ...
  # DES-3200-26 ревизи€ C1 3200-52, DES-3200-28/C1 (Last version)
...
sub edgecore_port_speed {
  ...
  if ($sw_info->{version} =~ /3528/) {
    ...
    #edge-core 3528 (скорость  от 64kbps до 100000 kbps кратна 1 бит/с)
  ...
  #11 - исход
  #24 - port
  #2 - scale ( {1, 2, 3, 4}; {8M, 800K, 80K, 8K} соответственно ), то есть скорость в битах задать нельз€, только в байтах - придетс€ как-то вычисл€ть
  #
  #
  #
  ...
  #10 - исход
  #24 - port
  #3 - level (множитель, умножаетс€ на scale дл€ вычислени€ скорости, 1-127)
}
