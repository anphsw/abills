...
BEGIN {
  ...
  $libpath = $Bin . '/../'; #assuming we are in /usr/abills/libexec/
  if ($Bin =~ m/\/abills(\/)/) {
...
use Users;
use Abills::SQL;
use Abills::Base qw(parse_arguments ip2int _bp);
use Abills::Nas::Mikrotik;
use Nas;
use Internet;

my $db = Abills::SQL->connect(@conf{qw/dbtype dbhost dbname dbuser dbpasswd/}, \%conf);
...
if ($user_nas) {
  ...
  if ($nas_info->{NAS_TYPE} eq 'mikrotik' || $nas_info->{NAS_TYPE} eq 'mikrotik_dhcp') {
    ...
    my Abills::Nas::Mikrotik $mikrotik = Abills::Nas::Mikrotik->new($nas_info, \%conf, { DEBUG => 0 });
