...
sub sync_status {
  ...
  $admin->query("SELECT u.id AS login, dv.disable AS status, u.gid, u.uid
                     FROM users u
  ...
  foreach my $line (@$list) {
    if ($master_info->{$line->{login}}) {
      while (my ($k, $v) = each %{$master_info->{$line->{login}}}) {
        if ($v ne $line->{lc($k)}) {
          if ($k eq 'GID') {
            #$admin->query("UPDATE users SET gid=? WHERE uid=?", 'do', { Bind => [ $v, $line->{uid} ] });
          }
          elsif ($k eq 'STATUS') {
            my $status = $status_{$v} || 0;
            #$admin->query("UPDATE dv_main SET disable=? WHERE uid=?", 'do', { Bind => [ $status, $line->{uid} ] });
          }
