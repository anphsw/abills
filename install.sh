...
RESULT=`grep WITHOUT_X11 /etc/make.conf`;
if [ "${RESUL}" = "" ]; then
...
for name in ${RESULT}; do
  ...
  if [ ${name} = \"MRTG\" ]; then
    ...
    cmd=${cmd}"fetch http://aarnet.dl.sourceforge.net/project/abills/Misc/rstat-0.19/rstat-0.19.tgz;"
    cmd=${cmd}"tar zxvf rstat-0.19.tgz;"
    cmd=${cmd}"cd rstat;"
    cmd=${cmd}"make install;"
...
case "${OS_NAME}" in
  *buntu)
    if [ w${REBUILD} != w ]; then
      ...
      BUILD_OPTIONS=" sudo apt-get install ";
    fi;

    _install dialog cvs make gcc vim bc
...
#      install_dhcp
  ...
  if [ ${name} = \"MRTG\" ]; then
    ...
    cmd=${cmd}"wget http://aarnet.dl.sourceforge.net/project/abills/Misc/rstat-0.19/rstat-0.19.tgz ;"
    cmd=${cmd}"tar zxvf rstat-0.19.tgz;"
    cmd=${cmd}"cd rstat;"
    cmd=${cmd}"make install;"
    AUTOCONF_PROGRAMS="${AUTOCONF_PROGRAMS} mrtg"
